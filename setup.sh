...
# Called from both interactive and non-interactive setup paths.
setup_knowledge_planes() {
	local repos_file="$HOME/.config/aidevops/repos.json"
	local helper
	# Prefer repo-local helper; fall back to deployed copy.
...
_setup_run_non_interactive() {
	...
	# Provision knowledge planes for all repos in repos.json where
	# knowledge != "off". Idempotent — existing directories are not modified.
	setup_knowledge_planes
	return 0
}
...
_setup_run_interactive() {
	...
	confirm_step "Provision knowledge planes (repos with knowledge: repo|personal)" && setup_knowledge_planes
	return 0
}
