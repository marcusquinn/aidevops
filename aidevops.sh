...
# Resolve the actual directory of aidevops.sh (symlink-safe).
# Sub-libraries (aidevops-repos.sh, aidevops-init.sh, aidevops-plugins.sh)
# live in the same directory as this script.
# BASH_SOURCE[0] gives the invocation path which may be a symlink — follow
# it to reach the actual script file and its sibling sub-libraries.
_ad_script="${BASH_SOURCE[0]}"
if command -v readlink &>/dev/null; then
	while [[ -L "$_ad_script" ]]; do
		_ad_dir="$(cd "$(dirname "$_ad_script")" && pwd)"
		_ad_script="$(readlink "$_ad_script")"
		[[ "$_ad_script" != /* ]] && _ad_script="$_ad_dir/$_ad_script"
	done
fi
_AIDEVOPS_SCRIPT_DIR="$(cd "$(dirname "$_ad_script")" && pwd)"
unset _ad_script
...
# Repo management functions: init_repos_file, register_repo, detect_unregistered_repo, etc.
# shellcheck source=./aidevops-repos.sh
# shellcheck disable=SC1091  # sub-library resolved at runtime via _AIDEVOPS_SCRIPT_DIR
source "${_AIDEVOPS_SCRIPT_DIR}/aidevops-repos.sh"
...
# Init and scaffold functions: scaffold_agents_md, cmd_init, etc.
# shellcheck source=./aidevops-init.sh
# shellcheck disable=SC1091  # sub-library resolved at runtime via _AIDEVOPS_SCRIPT_DIR
source "${_AIDEVOPS_SCRIPT_DIR}/aidevops-init.sh"
...
# Skills and plugin management: cmd_skill, cmd_skills, cmd_plugin, etc.
# shellcheck source=./aidevops-plugins.sh
# shellcheck disable=SC1091  # sub-library resolved at runtime via _AIDEVOPS_SCRIPT_DIR
source "${_AIDEVOPS_SCRIPT_DIR}/aidevops-plugins.sh"
