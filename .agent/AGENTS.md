...
| **Cursor** | Reads `.cursor/rules/` symlinks → `.agent/` subfolders |
...
**Note**: Symlinks like `.ai`, `.kiro`, `.continue` are not tracked in git as they cause duplicate `@` references in OpenCode. Create them locally if needed for specific tools.
