...
# =============================================================================
# aidevops Skills & Plugin Management Library
# =============================================================================
# Skill and plugin management functions extracted from aidevops.sh to keep
# the orchestrator under 2000 lines.
#
# Covers:
#   - cmd_skill: import/manage individual skills (add/list/check/update/remove)
#   - cmd_skills: discover and browse the skills registry
#   - cmd_plugin: manage third-party agent plugins (add/list/enable/disable/remove)
#   - Plugin lifecycle helpers: _plugin_add, _plugin_list, _plugin_update,
#     _plugin_toggle, _plugin_remove, _plugin_scaffold
#
# Usage: source "${_AIDEVOPS_SCRIPT_DIR}/aidevops-plugins.sh"
#
# Dependencies:
#   - print_info/success/warning/error/header (defined in aidevops.sh)
#   - AGENTS_DIR, CONFIG_DIR (set by aidevops.sh)
#
...
# Include guard
[[ -n "${_AIDEVOPS_PLUGINS_LIB_LOADED:-}" ]] && return 0
_AIDEVOPS_PLUGINS_LIB_LOADED=1
...
cmd_plugin() {
	...
	enable)
		...
		local _plugin_target="$1"
		_plugin_toggle "$pf" "$ad" "$_plugin_target" enable
		;;
	disable)
		[[ $# -lt 1 ]] && {
			print_error "Plugin name required"
		...
		local _plugin_target="$1"
		_plugin_toggle "$pf" "$ad" "$_plugin_target" disable
		;;
	remove | rm)
		[[ $# -lt 1 ]] && {
			print_error "Plugin name required"
		...
		local _plugin_target="$1"
		_plugin_remove "$pf" "$ad" "$_plugin_target"
		;;
