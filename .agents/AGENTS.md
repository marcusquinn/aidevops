...
- **`### Complexity Impact` field (t2803):** Section in the brief template (nested under `## How`) for tasks that modify existing shell functions. Requires the brief author to measure the target function's current line count, estimate growth, and — when projected post-change length exceeds 80% of the 100-line `function-complexity` gate — plan an extract-helpers refactor BEFORE writing implementation steps. Prevents the recurring pattern where workers grow a function past the gate threshold and trigger repeated dispatch failures (canonical: 8 workers on GH#20702). Include this section for any `EDIT:` targeting an existing function body; delete it when the task creates only new files or new functions. Full guidance: `reference/large-file-split.md §0`.
