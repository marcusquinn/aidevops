...
**Auto-claim on worktree creation (GH#20102):** `worktree-helper.sh add|switch` and `full-loop-helper.sh start` automatically call `interactive-session-helper.sh claim` when the branch name encodes a task ID (`feature/tNNN-*`) or issue number (`feature/ghNNN-*`). This closes the race window between worktree creation and manual claim. Suppress with `AIDEVOPS_SKIP_AUTO_CLAIM=1` for scripted bulk operations. Headless sessions (`FULL_LOOP_HEADLESS`, `AIDEVOPS_HEADLESS`, `Claude_HEADLESS`, `GITHUB_ACTIONS`) are always skipped.
