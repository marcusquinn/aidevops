...
**Client-side pre-push guards (t1965, t2198, t2745, t3224):** Five opt-in `pre-push` hooks: **privacy** (blocks private repo slugs in public commits), **complexity** (blocks new violations of function/nesting/file size limits), **scope** (blocks out-of-scope file changes per brief `Files Scope`), **dup-todo** (blocks pushes where `TODO.md` has duplicate task-ID checkbox lines), **repo-verify** (runs the target repo's declared `format`/`lint`/`typecheck` before push, with optional auto-fix-and-amend in headless contexts). Install: `install-pre-push-guards.sh install`. Bypass all: `git push --no-verify`. Full detail and individual bypass flags: `reference/pre-push-guards.md`.
