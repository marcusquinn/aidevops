...
- Interactive issue pickup: when the user identifies an existing issue or you open a worktree for one, immediately run `interactive-session-helper.sh claim <N> <owner/repo>` to self-assign, apply `status:in-review`, and write the crash-recovery stamp; release with `interactive-session-helper.sh release <N> <owner/repo>` on handoff, abandonment, or task switch. Use `lockdown` when dispatch-only insulation is insufficient; never remove another claim's worktree because it can destroy an active agent workspace.
...
- Auto-generated issue triage outcomes: verify premise first; falsified → close with rationale; correct+obvious → implement+PR; correct+ambiguous → decision-ready comment + `needs-maintainer-review`. Scope/style uncertainty is not NMR. Full templates: `reference/worker-discipline.md`.
