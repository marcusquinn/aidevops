...
- With safe work and execution authority, continue through verification in the same session/worktree; defer only for a blocker, unrelated objective, or explicit parallel/background request. Approaching a context limit is neither completion nor a blocker: checkpoint, compact or roll over, then resume the exact next safe action without a progress-only stop. See `reference/session.md`.
