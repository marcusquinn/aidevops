...
- **`## Files Scope` field:** Section in the brief template for declaring allowed file paths (globs supported). The `scope-guard-pre-push.sh` hook uses this to block out-of-scope pushes, preventing accidental scope-leak. One path or glob per `- ` line.
...
**REST fallback for GraphQL-exhausted gh wrappers (t2574, GH#20243):** `gh issue create|comment|edit` commands route internally through GitHub's GraphQL API, which has a 5000-point/hour budget separate from the core REST budget. When cross-runner activity saturates GraphQL (observed 2026-04-21 at 0/5000 remaining for ~45 minutes), the framework's wrappers in `shared-gh-wrappers.sh` now automatically retry via `gh api -X POST|PATCH /repos/...` REST endpoints — so workers no longer crash at step 1 during exhaustion windows. The fallback activates when the primary call fails AND `gh api rate_limit` reports GraphQL remaining ≤ 10 (configurable via `AIDEVOPS_GH_REST_FALLBACK_THRESHOLD`). Emits `[INFO] gh-wrapper: GraphQL exhausted, falling back to REST for <endpoint>` so the symptom is visible in logs. Covers `gh_create_issue`, `gh_issue_comment`, `gh_issue_edit_safe`, and `set_issue_status`. Translators live in `shared-gh-wrappers-rest-fallback.sh`; label and assignee PATCHes compute the full target array (REST requires the full set, not add/remove deltas) by fetching current state via REST. Test coverage: `.agents/scripts/tests/test-gh-wrapper-rest-fallback.sh`.
