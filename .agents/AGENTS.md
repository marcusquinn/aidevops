...
**`status:available` default safety net (t2798):** Issues created via bare `gh issue create` (bypassing `claim-task-id.sh`) may lack a `status:*` label, making them invisible to the pulse's dispatch fill-floor. The `.github/workflows/apply-status-available-default.yml` workflow catches this: on `issues.opened` and `issues.labeled`, it applies `status:available` when the issue has `auto-dispatch`, no existing `status:*` label, and no excluded label (`parent-task`, `persistent`, `consolidation-task`, `routine-tracking`, `no-auto-dispatch`). The `claim-task-id.sh` path remains the primary source of the default; this workflow is the server-side safety net.
