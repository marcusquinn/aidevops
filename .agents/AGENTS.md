...
**Main-branch-guard post-checkout hook (t1994):** A git `post-checkout` hook at `.agents/hooks/main-branch-guard-post-checkout.sh` enforces "canonical worktree stays on main" at the git-operation level. When a branch-level checkout happens in the canonical worktree (detected via `git-dir == git-common-dir`), the hook auto-restores `main` and prints a guided error directing the user to `wt add <branch>`. Linked worktrees are unaffected. Install per-repo with `.agents/scripts/install-main-branch-guard.sh install`. Status/uninstall: same script, `status`/`uninstall`. Installed automatically by `setup.sh` across all `initialized_repos[]` entries. Bypass for legitimate cases: `AIDEVOPS_MAIN_BRANCH_GUARD=false git checkout ...`. Diagnose with: `install-main-branch-guard.sh status`. Test harness: `.agents/scripts/test-main-branch-guard.sh`.
