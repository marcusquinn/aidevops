...
**`## Files Scope` field**: The brief template includes a `## Files Scope` section where the author declares the exact file paths (or glob patterns) the task is allowed to modify. The `scope-guard-pre-push.sh` hook reads this section to block pushes that touch files outside the declared scope, providing automated enforcement against rebase-induced scope drift (see parent #19808).
