...
### Dispatch-path default (t2821, t2832)

Tasks that modify the worker dispatch/spawn path have a tautology failure mode: broken dispatch kills the workers sent to fix dispatch. Default these to `no-auto-dispatch` + `origin:interactive` for maintainer implementation. Add `#parent` only when the task is also a genuine decomposition tracker — as of t2832, `no-auto-dispatch` alone is an unconditional dispatch block.
...
- **Default:** recommend `no-auto-dispatch` + `origin:interactive` in the TODO entry. `dispatch-dedup-helper.sh::_is_assigned_check_no_auto_dispatch` short-circuits with `NO_AUTO_DISPATCH_BLOCKED` upstream of NMR auto-approval (t2832), so the label alone is sufficient — add `#parent` only when the issue is also a genuine decomposition tracker. The previous default (`#parent` + `no-auto-dispatch`) was a workaround for the dispatch-path enforcement gap and is no longer required. `task-brief-helper.sh` appends a `## Dispatch-Path Classification` notice to the generated brief. `claim-task-id.sh` emits a non-blocking stderr warning when `--labels auto-dispatch` is passed on a dispatch-path task.
