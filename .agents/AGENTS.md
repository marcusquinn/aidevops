...
**`#auto-dispatch` skips `origin:interactive` self-assignment (t2157, t2406, t2218)**: When any of the three issue-creation paths — `issue-sync-helper.sh` (TODO-first push), `gh_create_issue` (direct wrapper), or `claim-task-id.sh` (agent-claimed follow-up) — creates an issue tagged `#auto-dispatch`, the pusher is NOT self-assigned even when the session origin is `interactive`. The `#auto-dispatch` tag signals "let a worker handle this" — self-assignment would create the `(origin:interactive + assigned + active status)` combo that GH#18352/t1996 treats as a permanent dispatch block, stranding the issue until manual `gh issue edit --remove-assignee` or the 24h `STAMPLESS_INTERACTIVE_AGE_THRESHOLD` safety net (t2148). An `[INFO]` log line is emitted when the skip fires in each path (`Skipping auto-assign for #N — auto-dispatch entry is worker-owned`). Regression tests: `.agents/scripts/tests/test-auto-dispatch-no-assign.sh` (`issue-sync-helper.sh` path), `.agents/scripts/tests/test-gh-create-issue-auto-dispatch-skip.sh` (`gh_create_issue` path), `.agents/scripts/tests/test-claim-task-id-autodispatch.sh` (`claim-task-id.sh` path). For issues already created before the carve-out or from code paths that predate it, `interactive-session-helper.sh post-merge <PR>` (t2225) automates the heal across every issue referenced in a just-merged PR.
...
- **Applied synchronously at creation (t2436)** — `claim-task-id.sh` scans TODO.md for the task ID at issue creation time and derives `parent-task` (and all other tag-derived labels) via `map_tags_to_labels()` BEFORE the issue is created. `gh_create_issue()` does the same via auto-detection of the `tNNN:` title prefix. This closes the race window where a peer runner could dispatch a worker in the 30-60s gap between issue creation and the asynchronous `issue-sync.yml` workflow. A secondary `DISPATCH_HYDRATION_WINDOW_S` grace period (default 30s) in `dispatch-dedup-helper.sh` catches any remaining edge cases. Test coverage: `.agents/scripts/tests/test-parent-tag-sync.sh`.
...
**Parent-task decomposition lifecycle (t2442):** A `parent-task` label is a *permanent dispatch block* — it must be paired with a concrete plan for how the work will be done, or it becomes backlog rot. Four cooperating mechanisms in `pulse-issue-reconcile.sh` and `issue-sync-lib.sh` enforce this:

1. **No-markers warn at label-application time (Fix #2).** When `parent-task` is applied to an issue whose body contains no recognised decomposition heading — `## Children`, `## Child Issues`, `## Sub-tasks`, `## Phases`, or `## Phase N[: ...]` — `_post_parent_task_no_markers_warning` (in `issue-sync-lib.sh`) posts a one-time comment pointing the author at the four remediation paths. Called from both `issue-sync-helper.sh cmd_push` and `claim-task-id.sh create_github_issue`. Dedup via `<!-- parent-task-no-markers-warning -->` marker.

2. **Prose-pattern children extraction (Fix #3).** `_extract_children_from_prose` in `pulse-issue-reconcile.sh` is the third fallback (after graph-based and heading-scoped extraction) for recognising decomposition in issue bodies that don't use formal headings. Narrow patterns only: `Phase N ... #NNNN`, `filed as #NNNN`, `tracks #NNNN`, `Blocked by: #NNNN`. Widening this set is forbidden (see t2244 — CodeRabbit disqualified "any `#NNN` = child" matching in PR #19810).

3. **24h advisory nudge (t2388, existing).** `reconcile_completed_parent_tasks` posts `_post_parent_task_decomposition_nudge` on parent-tasks with 0 children where ≥24h have elapsed since label application. Dedup via `<!-- parent-task-decomposition-nudge -->` marker.

4. **Auto-decomposer scanner (Fix #1, tightened t2573).** `auto-decomposer-scanner.sh` runs on every pulse cycle via `_run_auto_decomposer_scanner` (in `pulse-simplification.sh`) — the former global 24h run gate was removed in t2573 to allow multiple parents to be cleared per day. For each open parent-task with 0 children and an eligible nudge, it files a fresh `tier:thinking` worker issue with generator marker `<!-- aidevops:generator=auto-decompose -->`. Two age thresholds apply: **fresh parents** (0 non-nudge comments) require ≥`SCANNER_FRESH_PARENT_HOURS` (default 6h); **aged parents** (≥1 non-nudge comments) require ≥`SCANNER_NUDGE_AGE_HOURS` (default 24h). Per-parent state file (`AUTO_DECOMPOSER_PARENT_STATE`) prevents re-filing the same parent within `AUTO_DECOMPOSER_INTERVAL` (default 7 days). The worker's job is to read the parent, propose a decomposition plan as children, and stop. Dedup via `source:auto-decomposer` label. Constants in `pulse-wrapper.sh`: `AUTO_DECOMPOSER_INTERVAL` (per-parent re-file gate, 7 days), `AUTO_DECOMPOSER_PARENT_STATE`. Maintainer-only (skips `role: contributor` repos).

5. **7-day NMR escalation (Fix #4).** If the advisory nudge is ≥7 days old and still zero children exist, `_post_parent_decomposition_escalation` applies `needs-maintainer-review` and posts a comment listing the four paths forward: decompose into children, drop the `parent-task` label, close the issue, or let the auto-decomposer handle it. **Escalation never removes `parent-task`** — that would defeat the only reliable dispatch block (per t2211). The label removal instruction appears only as user-facing guidance inside a markdown code fence. Env override: `PARENT_DECOMPOSITION_ESCALATION_HOURS` (default 168). Capped at `max_escalations` per reconcile pass. Dedup via `<!-- parent-needs-decomposition-escalated -->` marker.

Test coverage: `.agents/scripts/tests/test-parent-prose-child-detection.sh`, `test-parent-task-application-warn.sh`, `test-parent-decomposition-escalation.sh`, `test-auto-decomposer-scanner.sh`, `test-auto-decomposer-per-parent-gate.sh` (84+ structural assertions total).
...
- **Release is the agent's responsibility**, not the user's. Call `interactive-session-helper.sh release <N> <slug>` when the user signals completion ("done", "ship it", "moving on", "let a worker take over") or when they switch to a different issue. The user should never need to type a release command. **PR merge auto-release (t2413, t2429):** when either `pulse-merge.sh` or `full-loop-helper.sh merge` merges an `origin:interactive` PR with a `Resolves #NNN` link and a claim stamp exists, `release_interactive_claim_on_merge` (from `shared-claim-lifecycle.sh`) fires automatically — no manual release required on merge. Manual release is still needed for task abandonment or mid-stream task switches.
...
**REST fallback for GraphQL-exhausted gh wrappers (t2574, GH#20243):** `gh issue create|comment|edit` and `gh pr create` commands route internally through GitHub's GraphQL API, which has a 5000-point/hour budget separate from the core REST budget. When cross-runner activity saturates GraphQL (observed 2026-04-21 at 0/5000 remaining for ~45 minutes), the framework's wrappers in `shared-gh-wrappers.sh` now automatically retry via `gh api -X POST|PATCH /repos/...` REST endpoints — so workers no longer crash at step 1 during exhaustion windows. The fallback activates when the primary call fails AND `gh api rate_limit` reports GraphQL remaining ≤ 10 (configurable via `AIDEVOPS_GH_REST_FALLBACK_THRESHOLD`). Emits `[INFO] gh-wrapper: GraphQL exhausted, falling back to REST for <endpoint>` so the symptom is visible in logs. Covers `gh_create_issue`, `gh_create_pr`, `gh_issue_comment`, `gh_issue_edit_safe`, and `set_issue_status`. Translators live in `shared-gh-wrappers-rest-fallback.sh`; label and assignee PATCHes compute the full target array (REST requires the full set, not add/remove deltas) by fetching current state via REST. For PR creation, labels are applied via a separate `POST /repos/{owner}/{repo}/issues/{pr_number}/labels` call since the pulls endpoint does not accept labels at creation time. Test coverage: `.agents/scripts/tests/test-gh-wrapper-rest-fallback.sh`.

**Pulse-level GraphQL rate-limit circuit breaker (t2690, GH#20310):** Proactive defence: pauses ALL worker dispatch when GitHub GraphQL budget drops below a configurable threshold, preventing cascade failures where the pulse spawns workers that fail at step 1 and burn $0.05-$0.25 each. Complements the reactive REST fallback layers (t2574 writes, t2689 reads). Helper: `scripts/pulse-rate-limit-circuit-breaker.sh` (`check | status`). Integrated into `_dff_compute_capacity` in `pulse-dispatch-engine.sh` — one check that gates the entire deterministic fill floor. Queries `gh api rate_limit` (free endpoint, no quota cost) and trips when `remaining / limit <= threshold`. Auto-resets when budget recovers on the next cycle. Fail-open on API errors. Counter: `pulse_dispatch_circuit_broken` in `~/.aidevops/logs/pulse-stats.json`, surfaced by `aidevops status`. Env controls: `AIDEVOPS_PULSE_CIRCUIT_BREAKER_THRESHOLD` (default `0.05` = 5% = 250/5000; set to `0` to disable), `AIDEVOPS_SKIP_PULSE_CIRCUIT_BREAKER=1` (emergency bypass). Multi-runner: each runner polls independently (same GitHub token = same budget response). Test coverage: `.agents/scripts/tests/test-rate-limit-circuit-breaker.sh` (15 assertions).

**Pulse decision correlation (t2714, GH#20372):** `pulse-diagnose-helper.sh pr <N> [--repo <slug>]` correlates `pulse.log` entries with `gh pr view` metadata to produce a structured "what did the pulse do on PR #N and why?" report. Each log line is classified against a 60+ rule inventory extracted from `pulse-merge.sh`, `pulse-wrapper.sh`, `pulse-dirty-pr-sweep.sh`, `pulse-merge-conflict.sh`, and `pulse-merge-feedback.sh`. Handles rotated logs, offline gh, zero-entry PRs (admin bypass). Use `--verbose` for raw log lines, `--json` for programmatic output, `rules` subcommand to list the full inventory. Test coverage: `.agents/scripts/tests/test-pulse-diagnose-helper.sh`.
...
**Client-side pre-push guards (t1965, t2198):** Three opt-in `pre-push` hooks block common mistakes before they hit CI. All three are installed by `.agents/scripts/install-pre-push-guards.sh install` (installs all) or `--guard privacy` / `--guard complexity` / `--guard scope` for individual guards. Status: `install-pre-push-guards.sh status`. Bypass all: `git push --no-verify`.
...
- **Scope guard** (`.agents/hooks/scope-guard-pre-push.sh`): blocks pushes that modify files outside the brief's declared `### Files Scope` section (or `## Files Scope` in older briefs), preventing accidental scope-leak during rebase or implementation drift (t2445, GH#19808). Reads the brief file for the current branch's task ID and enforces the declared glob patterns. Bypass: `SCOPE_GUARD_DISABLE=1 git push ...`. Fail-open when the branch name has no task ID or no brief file exists. Fail-closed when the brief exists but has no `Files Scope` section or the section is empty — a missing/empty section in an existing brief is a configuration error that must be fixed before pushing.
