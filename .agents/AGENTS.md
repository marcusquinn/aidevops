...
**Known limitation — sync-on-pr-merge TODO auto-completion (t2029):** The `sync-on-pr-merge` job in `.github/workflows/issue-sync.yml` tries to auto-mark TODO entries complete on PR merge, but its push to `main` is rejected by branch protection on personal-account repos (`required_approving_review_count: 1` + no bypass support for `github-actions[bot]` on classic protection — the `bypass_pull_request_allowances` API returns HTTP 500 on this plan, re-verified 2026-04-13). Since t2029 the workflow fails loudly (workflow error + PR comment) instead of silently when this happens. If you merge a PR and see a "TODO.md auto-completion blocked" comment, run the command in the comment (`task-complete-helper.sh tNNN --pr NNN`) and push. **Real fix (decided in t2038, GH#18522):** add a fine-grained PAT (`SYNC_PAT`, repo-scoped, `Contents: Read and write`) as a repo Actions secret and switch the `Checkout main` + push steps to use it — the PAT authenticates as the admin user, which bypasses `required_pull_request_reviews` via `enforce_admins: false`. Implementation tracked in **t2048 (GH#18643)**. The rulesets + Integration bypass alternative was disqualified: `bypass_actors` with `actor_type: "Integration"` returns `422 Validation Failed — Actor GitHub Actions integration must be part of the ruleset source or owner organization` on personal-account repos (no owner organization to attach to).
...
- `review_gate` — per-repo review gate configuration (t2123, GH#19173). Controls what happens when review bots are rate-limited during merge. Object with: `rate_limit_behavior` (`"pass"` default — exit 0, or `"wait"` — keep polling), and `tools` (per-tool overrides keyed by bot login: `coderabbitai`, `gemini-code-assist`, `augment-code`, `augmentcode`, `copilot`). Example: `{"rate_limit_behavior": "pass", "tools": {"coderabbitai": {"rate_limit_behavior": "wait"}}}`. Resolution: per-tool > per-repo > `REVIEW_GATE_RATE_LIMIT_BEHAVIOR` env var > `"pass"`.
