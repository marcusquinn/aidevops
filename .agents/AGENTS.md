...
4. **Auto-decomposer scanner (Fix #1).** `auto-decomposer-scanner.sh` runs every 24h via `_run_auto_decomposer_scanner` (in `pulse-simplification.sh`). For each open parent-task with 0 children and an advisory nudge ≥24h old, it files a fresh `tier:thinking` worker issue with generator marker `<!-- aidevops:generator=auto-decompose -->`. The worker's job is to read the parent, propose a decomposition plan as children, and stop. Dedup via `source:auto-decomposer` label. Constants in `pulse-wrapper.sh`: `AUTO_DECOMPOSER_LAST_RUN`, `AUTO_DECOMPOSER_INTERVAL`. Maintainer-only (skips `role: contributor` repos).
...
Test coverage: `.agents/scripts/tests/test-parent-prose-child-detection.sh`, `test-parent-task-application-warn.sh`, `test-parent-decomposition-escalation.sh`, `test-auto-decomposer-scanner.sh` (84 structural assertions total).
