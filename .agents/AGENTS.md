...
**Complexity Bump Override (t2370):** The `complexity-bump-ok` label overrides the per-metric complexity regression gates in `code-quality.yml` (function complexity, nesting depth, file size, bash32 compat). Unlike `ratchet-bump` (maintainer-only) and `new-file-smell-ok` (maintainer-only), `complexity-bump-ok` can be **self-applied by workers** when the PR body contains a validated `## Complexity Bump Justification` section. This is enforced by `.github/workflows/complexity-bump-justification-check.yml`, which triggers on the `labeled` event and validates that the section contains: (1) at least one `file:line` reference citing the specific location, (2) at least one numeric measurement (e.g. `depth=8`, `base=283 head=285`). If validation fails, the label is automatically removed and a remediation comment is posted. Workers completing file-split or simplification PRs should include the justification section in their PR body and apply the label — no maintainer intervention required. The existing `code-quality.yml` gates trust the label presence; this workflow enforces the evidence quality at label-apply time.
