...
- **REST fallback (t2574, t2744, t2902, t3459):** `shared-gh-wrappers.sh` routes eligible reads/writes through REST when GraphQL remaining <= 1500, preserving GraphQL for calls without REST equivalents. Eligible `gh_issue_view`, `gh_issue_list`, and `gh_pr_list` reads route to REST proactively before spending a GraphQL-backed `gh` call. Issue search uses `/search/issues` to preserve search semantics; PR `--search` remains GraphQL-only because the REST pulls endpoint has no equivalent. Overrides: `AIDEVOPS_GH_REST_FALLBACK_THRESHOLD`, `AIDEVOPS_GH_REST_FALLBACK_CACHE_TTL`.
