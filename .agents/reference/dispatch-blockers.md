...
| `status:needs-info` | `pulse-repo-meta.sh`, `dispatch-dedup-helper.sh` `_is_assigned_check_needs_info`, `pulse-dispatch-core.sh`, `pulse-check-queue-scan.py`, `pulse-wrapper-cycle-gates.sh` | External-information wait. Candidate discovery, direct dispatch, worker assignment, and runnable queue accounting all block (`NEEDS_INFO_BLOCKED`) even if stale `status:available` and `auto-dispatch` labels coexist. `pulse-ancillary-dispatch.sh` removes it only after contributor-reply reconciliation; explicit interactive work may still proceed. |
...
| `NEEDS_INFO_BLOCKED` | 0 (blocked) | `status:needs-info` label — worker dispatch waits for contributor-reply reconciliation |
