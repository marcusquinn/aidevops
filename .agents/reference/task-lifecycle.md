...
**Auto-dispatch default**: Always add `#auto-dispatch` to worker-ready issues/tasks created by main agents or workers unless an exclusion applies. Readiness means the brief/body has file references, a clear deliverable, and acceptance criteria; missing readiness is a prompt to finish the brief or mark `#parent`, not to file an implementation issue without dispatch. See `workflows/plans.md` "Auto-Dispatch Tagging".
