...
**Auto-dispatch default**: Worker-ready implementation issues/tasks created by main agents or workers default to `#auto-dispatch`; readiness is the gate, not an opt-in. Add the tag once the brief/body has a clear deliverable, referenced files or patterns, automatable verification, and 2+ acceptance criteria. If readiness is missing, finish the brief first or mark the work `#parent`/blocked instead of filing a non-dispatchable implementation issue. See `workflows/plans.md` "Auto-Dispatch Tagging".
- **Exclusions**: Needs credentials, decomposition, human preference/approval, incomplete dependencies, or explicit user preference. Dispatch-path files are **not** excluded post-t2920; they auto-dispatch with opus-4-7 elevation. Canonical blocker label set: `reference/dispatch-blockers.md`.
