...
function getMcpRegistry() {
  ...
  return [
    ...
    },
    {
      ...
      eager: false,
      toolPattern: "freecad_*",
      globallyEnabled: false,
      activationAgent: "freecad",
      agentSource: ["tools", "design", "freecad.md"],
      activationGuidance: [
        "Require explicit operator approval, isolation, and an absolute command from the reviewed pinned FreeCAD adapter checkout; never install or launch FreeCAD automatically.",
        "Distinguish headless FreeCAD execution from GUI control and read model state back after every mutation.",
      ],
      modelTier: "standard",
      description: "Operator-provisioned FreeCAD MCP with fail-closed execution guards",
    },
    {
      ...
      eager: false,
      toolPattern: "davinci-resolve_*",
      globallyEnabled: false,
      activationAgent: "davinci-resolve",
      agentSource: ["tools", "video", "davinci-resolve.md"],
      activationGuidance: [
        "Require explicit operator approval, isolation, an already running supported Resolve environment, and a reviewed absolute adapter command.",
        "Offline database or XML mutation is excluded; inspect actual exports and read timeline state back after every mutation.",
      ],
      modelTier: "standard",
      description: "Operator-provisioned DaVinci Resolve MCP with guarded project access",
    },
    {
      name: "ableton-live",
      type: "local",
      ...
      eager: false,
      toolPattern: "ableton-live_*",
      globallyEnabled: false,
      activationAgent: "ableton-live",
      agentSource: ["tools", "audio", "ableton-live.md"],
      activationGuidance: [
        "Require explicit operator approval, isolation, telemetry opt-out, a supported Ableton Live environment, and a reviewed absolute adapter command.",
        "Never install or open Live, plugins, packs, control surfaces, or the adapter; read session state back after every mutation.",
      ],
      modelTier: "standard",
      description: "Operator-provisioned Ableton Live MCP with telemetry and execution guards",
    },
    {
      name: "backblaze-b2",
      type: "local",
