...
- **Tool**: `unstract_tool` — submits file, polls for completion, returns structured JSON
...
Example: "Process the document /tmp/invoice.pdf"

## Setup

Store credentials via `~/.aidevops/agents/scripts/setup-local-api-keys.sh` (preferred) or manually in `~/.config/aidevops/credentials.sh` (chmod 600). The MCP expects `API_BASE_URL` without a prefix — this matches the official Unstract spec.

### Option A: Cloud (Quick Start)

Sign up at https://unstract.com/start-for-free/ (14-day free trial). Create a Prompt Studio project, define extraction schema, deploy as API endpoint, then set:
...
Add keys in Unstract UI (Settings > Adapters) or run `unstract-helper.sh configure-llm`.
...
`unstract/mcp-server` Docker image has no telemetry. Self-hosted: disable frontend analytics with `REACT_APP_ENABLE_POSTHOG=false` in `frontend/.env`. Cloud API may collect server-side usage metrics — use self-hosted if this is a concern.
