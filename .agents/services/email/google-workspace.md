...
- **CLI**: `gws` (`@googleworkspace/cli`) — reads Google Discovery Service at runtime; all Workspace APIs
...
```bash
gws gmail +triage                          # unread inbox summary (read-only)
...
gws calendar +agenda --today               # today's events (read-only)
...
gws schema gmail.users.messages.list       # introspect any method
```
...
**Auth precedence**: `GOOGLE_WORKSPACE_CLI_TOKEN` (1) → `GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE` (2) → `gws auth login` encrypted (3) → `~/.config/gws/credentials.json` (4). Variables can also be set in a `.env` file.
...
**Preferred — encrypted export (AES-256-GCM):**
...
**Fallback — plaintext export (store content, not path):**

```bash
gws auth export --unmasked > credentials.json
cat credentials.json | aidevops secret set GWS_CREDENTIALS_JSON && rm credentials.json

# At runtime — write to temp file, clean up after
...
```bash
# Triage (read-only — never modifies mailbox)
gws gmail +triage                                        # 20 most recent unread
gws gmail +triage --max 5 --query 'from:boss@example.com'
gws gmail +triage --labels
gws gmail +triage --format json | jq '.[].subject'

# Send — confirm with user before executing
gws gmail +send --to alice@example.com --subject "Hello" --body "Hi Alice!"
gws gmail +send --to alice@example.com --subject "Report" --body "<b>See attached.</b>" \
  --html --cc bob@example.com --bcc archive@example.com
gws gmail +send --to alice@example.com --subject "Test" --body "Hi" --dry-run

# Reply / Reply-all / Forward — confirm with user before executing
# Get MESSAGE_ID: gws gmail +triage --format json | jq '.[0].id'
...
gws calendar +agenda --days 3 --calendar 'Work' --timezone America/New_York

# Create event — confirm with user before executing
gws calendar +insert --summary "Standup" \
  --start "2026-06-17T09:00:00-07:00" --end "2026-06-17T09:30:00-07:00" \
  --location "Room A" --description "Q2 review" \
...
# Create / update — confirm with user before executing
...
- **Never commit** `~/.config/gws/credentials.json` or exported credentials files; store via `aidevops secret set`
- Headless/CI: use encrypted export (AES-256-GCM); store credentials *content* in secret manager, not a file path
...
- **Model Armor**: use `--sanitize` or `GOOGLE_WORKSPACE_CLI_SANITIZE_TEMPLATE` to scan responses for prompt injection
