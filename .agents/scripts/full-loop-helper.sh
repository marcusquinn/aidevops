...
# Validate that a PR title contains a task ID (t318.2)
# Called before PR creation to ensure audit traceability.
# Returns 0 if valid, 1 if missing task ID (warning only — does not block).
validate_pr_task_id() {
	local pr_title="$1"
	local branch="${2:-}"
	...
	print_warning "PR title missing task ID (tNNN): '$pr_title'"
	if [[ -n "$branch" ]]; then
		print_warning "Branch '$branch' does not contain a task ID pattern"
	fi
	print_warning "Every PR should reference a task from TODO.md for traceability"
	return 1
}
...
run_pr_create_phase() {
	...
	# Build PR title and body from task context (t156/t158/t318.2)
	...
	if [[ -n "$task_id_match" ]]; then
		...
		if [[ -n "$task_desc" ]]; then
			...
			# t318.2: MANDATORY format — task ID at start of title for audit traceability
			pr_title="${task_id_match}: ${short_desc}"
		else
			pr_title="${task_id_match}: feat"
		fi
	else
		# t318.2: Warn when no task ID found — every PR should be traceable to a task
		print_warning "No task ID (tNNN) found in branch name '$branch'"
		print_warning "PR title will lack task ID. Create a TODO entry first for traceability."
	...
	# t318.2: Validate PR title contains task ID before creation
	validate_pr_task_id "$pr_title" "$branch" || true
