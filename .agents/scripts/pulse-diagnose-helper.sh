...
_render_issue_attempts_text() {
	...
	read -r attempt_count rate_limit_count active cooldown_secs next_epoch < <(
		printf '%s' "$attempt_summary_json" | jq -r '[.attempt_count // 0, .rate_limit_count // 0, .backoff_active // false, .cooldown_secs // 0, .next_eligible_epoch // 0] | @tsv' || \
			printf '0\t0\tfalse\t0\t0\n'
	)
