...
_issue_needs_consolidation() {
	...
	substantive_count=$(printf '%s' "$comments_json" | jq --argjson min "$min_chars" '
		# Combine all noise patterns into a single regex — reduces test() calls from 13 to 1.
		(
			...
			+ "|^## (Triage Review|Completion Summary|Large File Simplification Gate|Issue Consolidation Needed|Additional Review Feedback|Cascade Tier Escalation)"
		...
		[.[] | select(
			(.body | length) >= $min
			and (.user.type != "Bot")
			and (.body | test($noise_pattern) | not)
		)] | length
