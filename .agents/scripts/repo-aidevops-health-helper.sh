...
#      version, commit locally for local_only:true repos, or open a version-bump
#      PR for remote repos. Skipped if the repo has uncommitted changes.
#
...
# gh wrappers inject signature footers and handle gh pr create edge cases.
...
_open_version_bump_pr() {
	local slug="$1" branch_name="$2" default_branch="$3" target_version="$4" existing_pr pr_body pr_url
	existing_pr=$(gh pr list --repo "$slug" --head "$branch_name" --state open --json url --jq '.[0].url // empty' 2>/dev/null || true)
	if [[ -n "$existing_pr" ]]; then
		log_info "bumped: $slug → v${target_version} via existing PR $existing_pr"; printf '%s\n' "$existing_pr"
		return 0
	fi
	# shellcheck disable=SC2016  # backticks are intentional markdown literals.
	pr_body=$(printf '## Summary\n\nBump `.aidevops.json` to aidevops v%s.\n\n## Why\n\nThe repo-aidevops health routine detected stale framework metadata; this PR preserves branch protection by updating metadata through review instead of pushing directly to the default branch.\n\n## How to verify\n\nReview `.aidevops.json` and confirm `aidevops_version` is `v%s` without other changes.\n\nGenerated by: `repo-aidevops-health-helper.sh` r914.\n' "$target_version" "$target_version")
	if ! pr_url=$(gh_create_pr --repo "$slug" --title "chore: bump .aidevops.json to v${target_version}" --body "$pr_body" --head "$branch_name" --base "$default_branch" 2>&1); then
...
_bump_single_repo() {
	...
	local branch_name="chore/aidevops-version-v${target_version}"
	...
	if [[ "$local_only" != "true" ]]; then
		...
		if ! git -C "$repo_path" checkout -q -B "$branch_name" "$default_branch"; then
	...
	local tmp_adj
	tmp_adj=$(mktemp) || { git -C "$repo_path" checkout -q "$default_branch" >/dev/null 2>&1 || true; echo failed; return 0; }
	if ! jq --arg v "$target_version" '.aidevops_version = $v' "$adj_file" >"$tmp_adj" 2>/dev/null; then
		log_warn "bump failed ($slug): jq rewrite error"
		rm -f "$tmp_adj"; git -C "$repo_path" checkout -q "$default_branch" >/dev/null 2>&1 || true; echo failed
		return 0
	fi
	mv "$tmp_adj" "$adj_file"
	...
	if ! git -C "$repo_path" add .aidevops.json >/dev/null 2>&1 ||
		...
		git -C "$repo_path" checkout -q "$default_branch" >/dev/null 2>&1 || true; echo failed
		return 0
	fi

	# Remote repos must preserve branch protection: push a dedicated branch and
	# open a PR instead of pushing directly to the default branch.
	if [[ "$local_only" != "true" ]]; then
		if ! git -C "$repo_path" push -u --force-with-lease origin "$branch_name" >/dev/null 2>&1; then
			log_warn "bump failed ($slug): branch push failed"
			git -C "$repo_path" checkout -q "$default_branch" >/dev/null 2>&1 || true
			echo failed
			return 0
		fi
		if ! _open_version_bump_pr "$slug" "$branch_name" "$default_branch" "$target_version" >/dev/null; then
			git -C "$repo_path" checkout -q "$default_branch" >/dev/null 2>&1 || true
			echo failed
			return 0
		fi
