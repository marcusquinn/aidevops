...
loop_set_state() {
	...
	# Use --arg for strings to prevent jq injection (GH#3798)
	if [[ "$value" =~ ^[0-9]+$ ]]; then
		# Integer
		...
		# String - use --arg to safely handle special characters
...
loop_block_task() {
	...
	local temp_file
	temp_file=$(mktemp)
	# Use --arg to safely handle special characters in task_id and reason (GH#3798)
	jq --arg task_id "$task_id" --arg reason "$reason" --arg blocked_at "$(date -u +"%Y-%m-%dT%H:%M:%SZ")" \
