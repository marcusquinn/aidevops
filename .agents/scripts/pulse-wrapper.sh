...
_update_quality_issue_body() {
	...
	debt_closed=$(gh issue list --repo "$repo_slug" \
		--label "quality-debt" --state closed --limit 200 \
		--json number --jq 'length' 2>/dev/null || echo "0")
	# Validate integers
	...
	if [[ -f "$scan_state_file" ]]; then
		prs_scanned_lifetime=$(jq -r '.scanned_prs | length // 0' "$scan_state_file" 2>/dev/null || echo "0")
		issues_created_lifetime=$(jq -r '.issues_created // 0' "$scan_state_file" 2>/dev/null || echo "0")
	fi
	...
	if [[ "$open_pr_count" -gt 0 && -x "$review_helper" ]]; then
		...
		while IFS= read -r pr_num; do
			[[ -z "$pr_num" ]] && continue
			local gate_result
			gate_result=$("$review_helper" check "$pr_num" "$repo_slug" 2>/dev/null || echo "UNKNOWN")
			case "$gate_result" in
			PASS*)
			...
			WAITING* | UNKNOWN*)
				...
				local pr_created
				pr_created=$(echo "$open_prs_json" | jq -r --argjson n "$pr_num" '.[] | select(.number == $n) | .createdAt' 2>/dev/null || echo "")
...
_Auto-updated by daily quality sweep. Comments below contain detailed findings per sweep. Do not edit manually._"
	...
	# Update issue body
	gh issue edit "$issue_number" --repo "$repo_slug" --body "$body" >/dev/null 2>&1 || {
		echo "[pulse-wrapper] Quality sweep: failed to update body on #${issue_number} in ${repo_slug}" >>"$LOGFILE"
		return 0
	}
	...
	local current_title
	current_title=$(gh issue view "$issue_number" --repo "$repo_slug" --json title --jq '.title' 2>/dev/null || echo "")
	if [[ "$current_title" != "$quality_title" ]]; then
		gh issue edit "$issue_number" --repo "$repo_slug" --title "$quality_title" >/dev/null 2>&1 || true
	fi
