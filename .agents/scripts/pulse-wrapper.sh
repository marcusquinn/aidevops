...
#######################################
# Check if a pulse session is active
# Returns: 0 if session active (proceed), 1 if not (skip)
#######################################
check_session_gate() {
	local session_flag="${HOME}/.aidevops/logs/pulse-session.flag"
	if [[ ! -f "$session_flag" ]]; then
		echo "[pulse-wrapper] No active pulse session — skipping cycle (start with: aidevops pulse start)" >>"$LOGFILE"
		return 1
	fi
	return 0
}
