...
cmd_record() {
	local op="" repo="" number="" caller_script="" caller_function="" caller_line="0"
	local before_json="" after_json="" flags_json="{}"
	...
	[[ -z "$before_json" ]] && before_json='{"title_len":0,"body_len":0,"labels":[]}'
	[[ -z "$after_json" ]] && after_json='{"title_len":0,"body_len":0,"labels":[]}'
	[[ -z "$caller_script" ]] && caller_script="unknown"
	[[ -z "$caller_function" ]] && caller_function="unknown"
	[[ ! "$caller_line" =~ ^[0-9]+$ ]] && caller_line=0

	# Extract before/after fields for delta computation
	...
	# Compute delta
	local title_delta_pct=0 body_delta_pct=0
	...
	while IFS= read -r diff_line; do
		case "$diff_line" in
		removed:*)
			removed_csv="${diff_line#removed:}"
			;;
		added:*)
			added_csv="${diff_line#added:}"
			;;
		esac
	...
	if command -v jq &>/dev/null; then
		...
		delta_json="{\"title_delta_pct\":${title_delta_pct},\"body_delta_pct\":${body_delta_pct},\"labels_removed\":${removed_json},\"labels_added\":${added_json}}"
	fi

	# Compute suspicious signals
	local suspicious_json
	suspicious_json="$(_gh_audit_compute_suspicious \
		"$title_delta_pct" "$body_delta_pct" "$removed_csv")"

	# Validate flags_json is parseable JSON
	if command -v jq &>/dev/null; then
		if ! printf '%s' "$flags_json" | jq -e '.' &>/dev/null; then
			flags_json="{}"
		fi
	fi

	# Build the NDJSON event
	local ts
	...
	if command -v jq &>/dev/null; then
		...
		event="{\"ts\":\"${ts}\",\"op\":\"${esc_op}\",\"repo\":\"${esc_repo}\",\"number\":${number},\"caller_script\":\"${esc_caller_script}\",\"caller_function\":\"${esc_caller_function}\",\"caller_line\":${caller_line},\"pid\":$$,\"flags\":${flags_json},\"before\":${before_json},\"after\":${after_json},\"delta\":${delta_json},\"suspicious\":${suspicious_json}}"
	fi
	...
	# Log to stderr when anomalies detected
	if [[ "$suspicious_json" != "[]" ]]; then
		_gh_audit_warn "ANOMALY detected in ${op} on ${repo}#${number}: ${suspicious_json}"
	else
		_gh_audit_info "Recorded ${op} on ${repo}#${number}"
	fi
...
cmd_status() {
	...
	if [[ "$entry_count" -gt 0 ]] && command -v jq &>/dev/null; then
		...
		local anomaly_count
		anomaly_count="$(jq 'select(.suspicious | length > 0)' "$log_file" 2>/dev/null | wc -l | tr -d '[:space:]')" || anomaly_count=0
