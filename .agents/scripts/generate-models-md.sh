...
generate_contest_results() {
	if ! [[ -f "$SCORING_DB" ]]; then
		echo "No contest data available yet. Run \`/compare-models\` or enable"
		echo "contest mode (t1011) to generate data."
		echo ""
		return 0
	fi
	...
	if [[ "$score_count" -gt 0 ]]; then
		echo "### Quality Scores"
		echo ""
		echo "Weighted average across all evaluated responses (correctness 30%, completeness"
		echo "25%, code quality 25%, clarity 20%):"
		echo ""
...
generate_global_md() {
...

	{
		...
		echo "Auto-generated by \`generate-models-md.sh --mode global\` —"
...
generate_performance_md() {
...

	{
		...
		echo ""
		echo "Per-repo performance data from pattern-tracker and response-scoring"
		echo "databases."
		echo "Auto-generated by \`generate-models-md.sh --mode performance\` —"
		echo "do not edit manually."
...
generate_models_md() {
...

	{
		...
		echo ""
		if [[ -n "$REPO_PATH" ]]; then
			echo "Per-repo performance data for **$(basename "$REPO_PATH")**"
			echo "from pattern-tracker and response-scoring databases."
		else
