...
QUIET=0
MODE="all"
REPO_FILTER="" # Optional: filter performance data by repo name (matches project_path)
...
generate_global_md() {
...

	{
		...
		echo "---"
		echo ""
		echo "*Generated by [aidevops](https://github.com/anomalyco/aidevops) t1012, t1133*"
	} >"$output"
...
generate_performance_md() {
...

	{
		...
		if [[ -n "$REPO_FILTER" ]]; then
			echo "Filtered by repo: \`$REPO_FILTER\`"
		fi
		echo ""
		echo "**Last updated**: $timestamp"
		echo ""
