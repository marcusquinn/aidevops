...
if [[ -z "${AIDEVOPS_PULSE_CIRCUIT_BREAKER_THRESHOLD+x}" ]] && [[ -f "$_PULSE_RL_CONF" ]]; then
	_PULSE_BATCH_SEARCH_LAST_RESORT_WAS_SET=0
	_PULSE_BATCH_SEARCH_LAST_RESORT_SAVED=""
	if [[ -n "${PULSE_BATCH_SEARCH_LAST_RESORT+x}" ]]; then
		_PULSE_BATCH_SEARCH_LAST_RESORT_WAS_SET=1
		_PULSE_BATCH_SEARCH_LAST_RESORT_SAVED="$PULSE_BATCH_SEARCH_LAST_RESORT"
	fi
	# shellcheck disable=SC1090
	source "$_PULSE_RL_CONF"
	if [[ "$_PULSE_BATCH_SEARCH_LAST_RESORT_WAS_SET" == "1" ]]; then
		PULSE_BATCH_SEARCH_LAST_RESORT="$_PULSE_BATCH_SEARCH_LAST_RESORT_SAVED"
	fi
	unset _PULSE_BATCH_SEARCH_LAST_RESORT_WAS_SET _PULSE_BATCH_SEARCH_LAST_RESORT_SAVED
fi
