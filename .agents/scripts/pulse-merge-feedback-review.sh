...
# Review-feedback evidence builders and redispatch routing.
...
_review_feedback_evidence_fingerprint() {
	...
	canonical=$(printf '%s\n%s\n' "$reviews_json" "$inline_json" | jq -csS --arg empty '' '
		.[0] as $reviews | .[1] as $inline |
		{
			version: 1,
			reviews: ($reviews | map({
				id: (.id // $empty | tostring), author: (.author // $empty),
				state: (.state // $empty), body: (.body // $empty), url: (.url // $empty),
				submitted_at: (.submitted_at // $empty), commit_id: (.commit_id // $empty)
			}) | sort_by(.id, .submitted_at, .commit_id)),
			inline: ($inline | map({
				id: (.id // $empty | tostring), author: (.author // $empty), path: (.path // $empty),
				line: (.line // 0), body: (.body // $empty), url: (.url // $empty),
				updated_at: (.updated_at // $empty), commit_id: (.commit_id // $empty)
