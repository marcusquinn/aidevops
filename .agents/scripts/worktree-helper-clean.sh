...
# =============================================================================
# Worktree Helper — Clean Sub-library
# =============================================================================
# Functions implementing the `worktree-helper.sh clean` command.
# Extracted from worktree-helper.sh to keep the orchestrator under 2000 lines.
#
# Usage: source "${SCRIPT_DIR}/worktree-helper-clean.sh"
#
# Dependencies (provided by the orchestrator before this file is sourced):
#   - shared-constants.sh  (color vars, is_worktree_owned_by_others,
#                            check_worktree_owner, unregister_worktree)
#   - audit-worktree-removal-helper.sh  (log_worktree_removal_event,
#                                        _WTAR_SKIPPED, _WTAR_REMOVED)
#   - _WTAR_WH_CALLER     (set in orchestrator, used in audit log calls)
#   - Functions from orchestrator: get_default_branch, branch_was_pushed,
#     _branch_exists_on_any_remote, worktree_has_changes, trash_path,
#     localdev_auto_branch_rm, assert_git_available, assert_main_worktree_sane
#
...
# Include guard
[[ -n "${_WORKTREE_HELPER_CLEAN_LIB_LOADED:-}" ]] && return 0
_WORKTREE_HELPER_CLEAN_LIB_LOADED=1
