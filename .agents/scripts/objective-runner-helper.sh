...
cmd_start() {
	...
	while [[ $# -gt 0 ]]; do
		...
		--max-steps)
			[[ $# -lt 2 ]] && {
				log_error "--max-steps requires a value"
				return 1
			}
			max_steps="$2"
			shift 2
		...
		--checkpoint-every)
			[[ $# -lt 2 ]] && {
				log_error "--checkpoint-every requires a value"
				return 1
			}
			checkpoint_every="$2"
			shift 2
		...
		--max-cost)
			[[ $# -lt 2 ]] && {
				log_error "--max-cost requires a value"
				return 1
			}
			max_cost="$2"
			shift 2
		...
		--max-tokens)
			[[ $# -lt 2 ]] && {
				log_error "--max-tokens requires a value"
				return 1
			}
			max_tokens="$2"
			shift 2
		...
		--allowed-paths)
			[[ $# -lt 2 ]] && {
				log_error "--allowed-paths requires a value"
				return 1
			}
			allowed_paths="$2"
			shift 2
		...
		--allowed-tools)
			[[ $# -lt 2 ]] && {
				log_error "--allowed-tools requires a value"
				return 1
			}
			allowed_tools="$2"
			shift 2
		...
		--runner)
			[[ $# -lt 2 ]] && {
				log_error "--runner requires a value"
				return 1
			}
			runner="$2"
			shift 2
