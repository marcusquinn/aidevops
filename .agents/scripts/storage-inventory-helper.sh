...
_storage_inventory_records() {
	...
	_storage_emit_record "opencode-data" "opencode" "OpenCode application data" "${AIDEVOPS_OPENCODE_DATA_DIR:-${HOME}/.local/share/opencode}" "joint" "$STORAGE_UNKNOWN" "runtime-aware maintenance only" "$STORAGE_UNKNOWN" "OpenCode ownership and active-session state require runtime-aware queries" "Use aidevops opencode-db report"
