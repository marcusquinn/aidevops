...
12. **Pipeline health**: Check the "Queue Health Metrics" section for Phase 3 throughput. If "Phase 3 last eval" is 0 or "no data" while there are queued/blocked/in-review tasks, the pipeline itself is broken — not individual tasks. Create a `create_task` targeting the aidevops repo to diagnose and fix the root cause in `.agents/scripts/supervisor/ai-lifecycle.sh` (the `gather_task_state()` and `process_ai_lifecycle()` functions). Use model `opus` and tag `#auto-dispatch`. This is the highest-priority action — nothing else in the pipeline works if Phase 3 is broken.
