...
cmd_draft() {
	...
	while [[ $# -gt 0 ]]; do
		...
		--length) length="$_nxt"; shift 2 ;;
		--cite) cite="$_nxt"; shift 2 ;;
	...
	# Validate tone
	case "$tone" in
	neutral | formal | conciliatory | firm) ;;
	*)
		# Check custom tones
		local tones_config
		...
		if [[ -n "$tones_config" ]]; then
			...
			if [[ "$valid" != "true" ]]; then
				print_error "Unknown tone: ${tone}. Available: neutral, formal, conciliatory, firm (or custom in _config/draft-tones.json)"
				return 1
			fi
		else
	...
	case "$length" in
	short | medium | long) ;;
	*) print_error "Unknown length: ${length}. Available: short, medium, long"; return 1 ;;
	esac
	...
	local full_case_id kind party_self
	full_case_id="$(echo "$dossier" | jq -r '.id')"
	kind="$(echo "$dossier" | jq -r '.kind')"
	party_self="$(echo "$dossier" | jq -r '(.parties[] | select(.role == "client") | .name) // (.parties[0].name) // "the client"')"

	# Read sources
	local sources_path="${case_dir}/${CASE_SOURCES_FILE}"
	local sources_json="[]"
	[[ -f "$sources_path" ]] && sources_json="$(jq '.' "$sources_path" 2>/dev/null)" || sources_json="[]"

	# Handle cross-case includes
	local cross_case_ids=""
	local include_id
	for include_id in "${include_cases[@]:-}"; do
	...
	# Determine max sensitivity tier
	local tier
	tier="$(_max_tier "$sources_json" "$repo_path")"

	# Get source IDs for excerpt collection
	local source_ids
	source_ids="$(echo "$sources_json" | jq -r '.[].id' 2>/dev/null)" || source_ids=""

	# Collect excerpts from sources
	local excerpts
	...
	if [[ -n "$max_tokens_override" ]]; then
		...
		max_tokens="$(_length_to_tokens "$length")"
	fi

	# Write prompt to temp file for LLM routing
	local prompt_file
	prompt_file="$(mktemp)"
	printf '%s\n' "$prompt" >"$prompt_file"
	...
	if [[ ! -f "$routing_helper" ]]; then
		print_error "llm-routing-helper.sh not found at: ${routing_helper}"
		rm -f "$prompt_file"
		return 1
	fi
	...
	local response=""
	local model_used="$_UNKNOWN"

	if [[ "${LLM_ROUTING_DRY_RUN:-0}" == "1" ]]; then
		# Dry-run mode for testing — generate mock response
		response="[MOCK DRAFT] This is a ${tone} draft for case ${full_case_id}.

The intent is: ${intent}

Based on the available evidence, the following points are relevant:

1. Point one [${source_ids%% *}]
2. Point two [${source_ids%% *}]

Drafted with reference to: all consulted sources."
		...
		response="$(bash "$routing_helper" route \
			...
			print_error "LLM routing failed. Tier: ${tier}. Check llm-routing-helper.sh status."
			rm -f "$prompt_file"
			return 1
		}
		# Extract model from audit log (best effort)
	...
	timestamp="$(_iso_ts)"
	local provenance
	provenance="$(_build_provenance_footer "$sources_json" "$repo_path" \
		"$model_used" "$timestamp" "$cross_case_ids")"

	# Compose full draft output
	local intent_slug
	...
	local cross_list="${cross_case_ids:-none}"
...
cross_case_includes: \"${cross_list}\"
---
...
${provenance}"
	...
	if [[ "$dry_run" == true ]]; then
		if [[ "$json_mode" == true ]]; then
			jq -n \
				...
				--arg body "$full_draft" \
				--arg sources "$source_list" \
				--arg cross_cases "$cross_list" \
				'{case_id:$case_id, intent:$intent, tone:$tone, length:$length,
				  model:$model, tier:$tier, generated_at:$generated_at,
				  sources_consulted:$sources, cross_case_includes:$cross_cases,
	...
	_timeline_append "$case_dir" "$_TASK_DRAFT" "$actor" \
		"Draft generated: ${intent} (tone: ${tone}, tier: ${tier})" \
		"${CASE_DRAFTS_DIR}/${ts_filename}-${intent_slug}.md"
	...
	if [[ "$json_mode" == true ]]; then
		jq -n \
			...
			'{case_id:$case_id, intent:$intent, tone:$tone, length:$length,
			  model:$model, tier:$tier, generated_at:$generated_at,
			  file:$file, sources_consulted:$sources,
			  cross_case_includes:$cross_cases, dry_run:false}'
	else
		print_success "Draft generated: ${draft_file}"
		echo "  Case:   ${full_case_id}"
		echo "  Intent: ${intent}"
		echo "  Tone:   ${tone}"
		echo "  Tier:   ${tier}"
...
cmd_revise() {
	...
	local draft_file="" feedback="" repo_path="" dry_run=false json_mode=false
	local max_tokens_override=""

	while [[ $# -gt 0 ]]; do
		local _cur="${1:-}" _nxt="${2:-}"
		case "$_cur" in
		--revise) draft_file="$_nxt"; shift 2 ;;
		--feedback) feedback="$_nxt"; shift 2 ;;
		--repo) repo_path="$_nxt"; shift 2 ;;
		--dry-run) dry_run=true; shift ;;
		--max-tokens) max_tokens_override="$_nxt"; shift 2 ;;
		--json) json_mode=true; shift ;;
		-*) print_error "Unknown option: $_cur"; return 1 ;;
		*) [[ -z "$draft_file" ]] && draft_file="$_cur" || true; shift ;;
		esac
	done
	...
	local case_id intent tone length
	...
	if [[ -z "$case_id" ]]; then
		print_error "Cannot extract case_id from draft frontmatter"
		return 1
	fi
	...
	# Read sources for tier determination
	local sources_path="${case_dir}/${CASE_SOURCES_FILE}"
	local sources_json="[]"
	[[ -f "$sources_path" ]] && sources_json="$(jq '.' "$sources_path" 2>/dev/null)" || sources_json="[]"
	...
	# Strip frontmatter from existing draft body (macOS sed compatible)
	local draft_body
	...
	revision_prompt="Revise this draft per the following feedback.
...
Preserve the original citation anchors [source-id]. Apply the feedback while maintaining factual accuracy and the same citation standard. Output only the revised draft text."

	# Write prompt to temp file
	local prompt_file
	prompt_file="$(mktemp)"
	printf '%s\n' "$revision_prompt" >"$prompt_file"
	# shellcheck disable=SC2064
	trap "rm -f '${prompt_file}'" EXIT
	...
	local routing_helper="${SCRIPT_DIR}/llm-routing-helper.sh"
	local response="" model_used="$_UNKNOWN"

	if [[ "${LLM_ROUTING_DRY_RUN:-0}" == "1" ]]; then
		response="[MOCK REVISION] Revised draft based on feedback: ${feedback}

${draft_body}

(Revisions applied per feedback.)"
		model_used="dry-run-mock"
	else
		response="$(bash "$routing_helper" route \
			--tier "$tier" \
			...
			--prompt-file "$prompt_file" \
			--max-tokens "$max_tokens" 2>/dev/null)" || {
			print_error "LLM routing failed for revision. Tier: ${tier}"
			rm -f "$prompt_file"
			return 1
		}
		model_used="${tier}-default"
	...
	# Count existing revisions for this intent
	local rev_count=2
	local existing_revs
...
${provenance}"
	...
	if [[ "$dry_run" == true ]]; then
		if [[ "$json_mode" == true ]]; then
			jq -n \
				...
				--arg revision "$rev_count" \
				--arg body "$full_revision" \
				'{case_id:$case_id, feedback:$feedback, revision:($revision|tonumber), body:$body, dry_run:true}'
		else
			printf '%s\n' "$full_revision"
		fi
		return 0
	...
	# Write revision file
	local rev_file
	rev_file="$(dirname "$draft_file")/${ts_filename}-${intent_slug}-rev${rev_count}.md"
	...
	_timeline_append "$case_dir" "draft-revision" "$actor" \
		"Draft revised (rev${rev_count}): ${intent}. Feedback: ${feedback}" \
		"${CASE_DRAFTS_DIR}/$(basename "$rev_file")"
	...
	if [[ "$json_mode" == true ]]; then
		jq -n \
			...
			--arg revision "$rev_count" \
			--arg file "$rev_file" \
			--arg original "$(basename "$draft_file")" \
			'{case_id:$case_id, feedback:$feedback, revision:($revision|tonumber),
			  file:$file, original:$original, dry_run:false}'
	else
		print_success "Revision generated: ${rev_file}"
		echo "  Case:      ${case_id}"
		echo "  Revision:  rev${rev_count}"
		echo "  Original:  $(basename "$draft_file")"
		echo "  File:      ${rev_file}"
	fi
	return 0
}
