...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.provider == "google-ads":
            credentials = (
                os.environ.get("GOOGLE_ADS_ACCESS_TOKEN", ""),
                os.environ.get("GOOGLE_ADS_DEVELOPER_TOKEN", ""),
            )
            result = collect_google(args.account_ref, args.start, args.end, credentials)
        else:
            result = collect_meta(
                args.account_ref, args.start, args.end, os.environ.get("META_ACCESS_TOKEN", "")
            )
        result.update(plan)
