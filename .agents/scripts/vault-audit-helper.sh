...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "audit_payload_key": b64e(secrets.token_bytes(32)),
    }
...
def cmd_append(args: argparse.Namespace) -> int:
    ...
    ciphertext = AESGCM(b64d(str(key["audit_payload_key"]))).encrypt(nonce, canonical(event_payload), b"aidevops-vault-audit-event")
    ...
    public_record = {
        ...
        "audit_public_key": str(key[FIELD_AUDIT_PUBLIC_KEY]),
    }
...
def verify_records(records: list[dict[str, Any]]) -> tuple[int, str]:
    ...
    for record in records:
        ...
        verify_signature(signature_payload, str(record.get(FIELD_SIGNATURE, "")), str(record.get("audit_public_key", "")))
...
def verify_receipt(path: Path, expected_head: str | None) -> None:
    receipt = load_json(path)
    body = receipt.get(FIELD_RECEIPT)
    if not isinstance(body, dict):
        raise AuditError("VAULT_AUDIT_RECEIPT_CORRUPTED", "Receipt body is missing", 3)
    if expected_head and body.get("observed_head") != expected_head:
        raise AuditError("VAULT_AUDIT_RECEIPT_MISMATCH", "Receipt does not match audit head", 6)
    verify_signature(body, str(receipt.get(FIELD_SIGNATURE, "")), str(body.get("observer_public_key", "")), "VAULT_AUDIT_BAD_RECEIPT_SIGNATURE")


def cmd_verify(args: argparse.Namespace) -> int:
    vault_dir = vault_dir_from(args.vault_dir)
    records = read_records(Path(args.log) if args.log else log_path(vault_dir))
    sequence, head = verify_records(records)
    for receipt_file in args.receipt or []:
        verify_receipt(Path(receipt_file), head)
...
def cmd_receipt(args: argparse.Namespace) -> int:
    ...
    body = {
        ...
        "observer_public_key": str(key[FIELD_AUDIT_PUBLIC_KEY]),
        "observed_head": validate_safe_name(FIELD_HEAD, args.head),
