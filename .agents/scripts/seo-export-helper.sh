...
main() {
	...
	gsc | bing | ahrefs | dataforseo)
		[[ -n "$domain" ]] || {
			print_error "Domain is required. Usage: seo-export-helper.sh $command <domain> [--days N]"
			return 1
		}
	...
	all)
		[[ -n "$domain" ]] || {
			print_error "Domain is required. Usage: seo-export-helper.sh all <domain> [--days N]"
			return 1
		}
		ensure_directories "$domain" >/dev/null
	...
	exports)
		[[ -n "$domain" ]] || {
			print_error "Domain is required. Usage: seo-export-helper.sh exports <domain>"
			return 1
		}
		list_exports "$domain"
		;;
