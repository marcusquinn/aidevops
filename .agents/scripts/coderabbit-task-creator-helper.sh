...
cmd_create() {
    ...
    while IFS= read -r finding; do
        ...
        if [[ "$dry_run" == "true" ]]; then
            echo "  [DRY RUN] $task_desc"
        else
            task_lines="${task_lines}- [ ] ${task_desc}\n"

            # Mark as task created in DB
            db "$TASK_CREATOR_DB" "
                UPDATE processed_findings SET task_created = 1 WHERE id = $finding_id;
            " 2>/dev/null || true
            ...
            db "$TASK_CREATOR_DB" "
                ...
                VALUES ($finding_id, 'auto-cr-$finding_id', '$escaped_desc', '$severity');
            " 2>/dev/null || true
        fi
...
cmd_verify() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --valid) verdict="valid"; shift ;;
            --false-positive) verdict="false_positive"; shift ;;
            *) log_warn "Unknown option: $1"; shift ;;
        esac
    done
