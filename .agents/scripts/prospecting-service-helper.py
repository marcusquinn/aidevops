...
from prospecting_api import OriginPolicy, ProspectingAPI
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _run(self) -> None:
        ...
        response = self.api.request(self.command, self.path, dict(self.headers.items()), raw)
...
def serve(args: argparse.Namespace) -> int:
    if args.bind not in LOOPBACK:
        if not all((args.allow_external, args.tls_cert, args.tls_key, args.public_host)):
            raise ValueError("external binding requires --allow-external, --public-host, --tls-cert, and --tls-key")
    ...
    policy = OriginPolicy(args.bind, args.public_host, "https" if args.tls_cert else "http")
    api = ProspectingAPI(database, auth_database, policy)
...
def start(args: argparse.Namespace) -> int:
    ...
    with os.fdopen(descriptor, "ab", closefd=True) as log:
        subprocess.Popen(  # nosec B603: fixed interpreter/helper; validated arguments, no shell
...
def parser() -> argparse.ArgumentParser:
    ...
    for name in ("serve", "start"):
        ...
        command.add_argument("--public-host", help="exact external Host allowed for owner requests")
