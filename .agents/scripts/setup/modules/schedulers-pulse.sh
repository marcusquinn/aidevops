...
_pulse_write_direct_opencode_shim() {
	local _opencode_bin="${1:-}"
	local _shim_dir="${HOME}/.local/bin"
	local _shim_path="${_shim_dir}/opencode"
	local _opencode_dir=""
...
export PATH="${_opencode_dir}:${HOME}/.local/bin:${HOME}/.aidevops/agents/scripts:/usr/local/bin:/usr/bin:/bin\${PATH:+:\$PATH}"
exec "$_opencode_bin" "\$@"
EOF
	chmod +x "$_shim_path" || return 1
	printf '%s' "$_shim_path"
	return 0
}
...
_resolve_pulse_runtime_binary() {
	...
	if [[ -n "$opencode_bin" ]]; then
		...
		if [[ "$opencode_bin" != "${HOME}/.local/bin/opencode" ]]; then
