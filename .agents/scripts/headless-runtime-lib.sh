...
# headless-runtime-lib.sh — Stable utilities for headless-runtime-helper.sh.
# Extracted to keep the helper under LARGE_FILE_LINE_THRESHOLD (2000), mirroring
# the issue-sync-lib.sh / issue-sync-helper.sh split. Sections: state DB, auth,
# backoff, output parsing, metrics, sandbox, worker contract, activity watchdog,
# DB merge, dispatch ledger/locks, failure reporting, canary, model choice,
# server detection + cmd builders. Caller defines shared globals (STATE_DIR,
# STATE_DB, OPENCODE_BIN_DEFAULT, etc.) before sourcing.

[[ "${_HEADLESS_RUNTIME_LIB_LOADED:-}" == "1" ]] && return 0
_HEADLESS_RUNTIME_LIB_LOADED=1
...
attempt_pool_recovery() {
	...
	# out from under it. Headless workers NEVER call pool rotation — they only
	# record backoff so the pre-dispatch check skips the dead provider on the
	# next cycle. Token rotation is an INTERACTIVE-ONLY operation.
	case "$provider" in
...
# --- 8. Activity watchdog (inline fallback) -----------------------------------

# Inline fallback used when worker-activity-watchdog.sh is missing.
# Polls the output file for size growth and kills the worker if it stalls.
#
# Args:
#   $1 - output file path
#   $2 - worker PID to kill on timeout
#   $3 - exit code file (written with 124 on timeout)
#   $4 - session_key (passed to _watchdog_kill for claim release)
...
# Extracted from _run_activity_watchdog for reuse by both phases and the
# standalone worker-activity-watchdog.sh script.
#
# Args: worker_pid exit_code_file output_file reason [session_key]
_watchdog_kill() {
	local worker_pid="$1"
...
# --- 10. Dispatch ledger / session locks --------------------------------------
...
#
# Previously, only the pulse and launchd watchdog wrote to the counter — both
# asynchronous, discovering failures 10-30 minutes after the worker died.
# This function lets the worker self-report immediately on exit, so escalation
# fires within seconds. The pulse path remains as a backup for hard crashes.
#
# Uses the same state file and locking as pulse-wrapper.sh and worker-watchdog.sh
# (fast-fail-counter.json + mkdir lock).
#
# Args:
#   $1 - session_key (e.g., "issue-marcusquinn-aidevops-17642")
#   $2 - failure reason (premature_exit, rate_limit, etc.)
#   $3 - crash_type (overwhelmed, no_work, or empty)
...
# --- 14. OpenCode server detection + cmd builders -----------------------------
...
# Outputs: null-delimited command tokens to stdout.
# Returns: 0 always.
_build_run_cmd() {
