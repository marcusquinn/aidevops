...
9. Model escalation before BLOCKED (GH#14964): BLOCKED is only valid after exhausting all autonomous solution paths. Before exiting BLOCKED, attempt escalation through the configured OpenAI tier resolver (for example, retry opus-tier work with --model openai/gpt-5.6-sol --variant xhigh). Review-policy metadata, nominal GitHub states, and lower-tier model limits are NOT valid blockers on their own.
