...
def analyze(data: Any) -> dict[str, Any]:
    ...
    for source in pages:
        ...
        for _, destination, anchor in sorted(scored, key=lambda item: (-item[0], item[1]["page_id"]))[:request["max_candidates_per_page"]]:
    ...
    return {"schema": REPORT_SCHEMA, "authority": "non_mutating_recommendations_only", "candidate_recall": {"max_candidates_per_page": request["max_candidates_per_page"], "method": "deterministic_observed_anchor"}, "link_proposals": proposals, "cannibalization": cannibalization}
