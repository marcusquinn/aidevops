...
# _resolve_quality_debt_assignee: resolve a reliable assignee for auto-created
# quality-debt issues so maintainer gate assignee checks do not deadlock fix PRs.
# Resolution order:
#   1) repos.json maintainer for this repo slug
#   2) authenticated gh user login
#   3) repo owner slug fallback (last resort)
# Arguments: $1=repo_slug
# Outputs assignee login to stdout.
_resolve_quality_debt_assignee() {
	local repo_slug="$1"
	local repos_json="${HOME}/.config/aidevops/repos.json"
	local assignee=""

	if [[ -f "$repos_json" ]]; then
		assignee=$(jq -r --arg slug "$repo_slug" \
...
_Auto-generated by \`quality-feedback-helper.sh scan-merged\`. Review each finding and either fix the code or dismiss with a reason._"
	...
	*) priority_label="" ;;
	...
	issue_assignee=$(_resolve_quality_debt_assignee "$repo_slug")
	...
	new_issue=$(gh issue create --repo "$repo_slug" \
		...
		--body "$issue_body" \
		--label "$label_args" \
		--assignee "$issue_assignee" | grep -oE '[0-9]+$' || echo "")
	...
	echo "  Failed to create quality-debt issue for ${file} (assignee: ${issue_assignee})" >&2
	echo "0"
	return 0
}
