...
cmd_scan_merged() {
	...
	# Load already-scanned PR numbers
	local scanned_prs
	scanned_prs=$(jq -r '.scanned_prs[]' "$state_file" 2>/dev/null | tr '\n' '|')
	...
	merged_prs=$(gh pr list --repo "$repo_slug" --state merged \
		...
		--jq 'sort_by(.mergedAt) | reverse | .[].number' 2>/dev/null) || {
	...
	while IFS= read -r pr_num; do
		...
		if echo "$scanned_prs" | grep -q "^${pr_num}|\\|^${pr_num}\$"; then
			continue
		fi
		prs_to_scan+=("$pr_num")
		count=$((count + 1))
	...
	if [[ ${#prs_to_scan[@]} -eq 0 ]]; then
		if [[ "$json_output" == "true" ]]; then
			echo '{"scanned":0,"findings":0,"issues_created":0}'
		else
...
_Auto-generated by \`quality-feedback-helper.sh scan-merged\`. Review each finding and either fix the code or dismiss with a reason._"
		...
		new_issue=$(gh issue create --repo "$repo_slug" \
			--title "$issue_title" \
			--body "$issue_body" \
			--label "quality-debt" 2>/dev/null | grep -oE '[0-9]+$' || echo "")
