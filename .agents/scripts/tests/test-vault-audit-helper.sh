...
"$VAULT_AUDIT_HELPER" verify --vault-dir "$vault_dir" --receipt "$receipt_file" >/dev/null
...
PY
if "$VAULT_AUDIT_HELPER" verify --vault-dir "$vault_dir" --receipt "$bad_receipt" >/dev/null 2>"$tmp_root/bad-receipt.err"; then
