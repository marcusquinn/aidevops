...
class PublicEngagementPolicyTests(unittest.TestCase):
    ...
    def test_exact_owner_approval_survives_delegated_revocation(self) -> None:
        store_owner_grant(self.database, self.grant, authenticated_owner_id="owner_fixture", current_time=100)
        self._operation("op_exact_override")
        self._authorize("op_exact_override")
        approve_operation(
            self.database, "op_exact_override", "owner_fixture", 200, approved_at=100
        )
        revoke_owner_grant(self.database, "grant_fixture", "owner_fixture", 101)
        claimed = claim_operation(
            self.database,
            ClaimRequest("op_exact_override", "owner_fixture", "executor", 102, 60),
        )
        mark_provider_started(self.database, claimed, "executor", started_at=103)
        started = self.database.execute(
            "SELECT provider_started_at FROM outbound_attempts WHERE attempt_id=?",
            (claimed.attempt_id,),
        ).fetchone()[0]
        self.assertEqual(started, 103)
    ...
    def test_stale_source_blocks_authorization_and_provider_start(self) -> None:
        grant = dict(self.grant, rules_max_age_seconds=10, expires_at=200)
        store_owner_grant(self.database, grant, authenticated_owner_id="owner_fixture", current_time=100)
        self._operation("op_stale_source")
        with self.assertRaisesRegex(SocialStoreError, "source evidence"):
            authorize_operation(
                self.database, "op_stale_source", "grant_fixture",
                project_id="project_fixture", corpus_id="corpus_fixture",
                community="aidevops_test", rules_observed_at=99,
                source_observed_at=50, current_time=100,
            )
        self._authorize("op_stale_source")
        claimed = claim_operation(
            self.database,
            ClaimRequest("op_stale_source", "owner_fixture", "executor", 100, 60),
        )
        with self.assertRaisesRegex(SocialStoreError, "stale"):
            mark_provider_started(self.database, claimed, "executor", started_at=111)
