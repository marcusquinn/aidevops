...
class RoutineTests(unittest.TestCase):
    ...
    def test_disabled_planning_and_cli_are_non_mutating(self):
        ...
        output = subprocess.run(  # nosec B603: fixed local test helper
            [sys.executable, str(SCRIPTS / "prospecting-routine-helper.py"), "plan", "--input", str(FIXTURE), "--dry-run"],
    ...
    def test_replay_reservation_and_unknown_delivery_do_not_duplicate(self):
        ...
        receipts = {}
        one = jobs.reserve(receipts, job)
        self.assertIs(one, jobs.reserve(receipts, job))
        rows = alerts.digest(self.document["leads"], set(), set())
        outbox = {}
        destination = {"kind": "webhook", "target": "https://alerts.example.test/hook"}
        entry = alerts.enqueue(outbox, "test-project", destination, "day-1", rows)
        alerts.transition(entry, "unknown")
        self.assertIs(entry, alerts.enqueue(outbox, "test-project", destination, "day-1", rows))
    ...
    def test_empty_and_unsafe_destinations_are_blocked(self):
        ...
        with self.assertRaises(alerts.AlertError):
            alerts.verified_destination({"kind": "webhook", "target": "http://127.0.0.1/x"})
