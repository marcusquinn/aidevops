...
class AccountSnapshotTests(unittest.TestCase):
    def test_google_uses_only_fixed_read_endpoint(self):
        captured = []
        credentials = ("token", "developer")
        result = google.collect("123-456-7890", "2026-01-01", "2026-01-02", credentials, lambda request: captured.append(request) or Response([{"campaign": {"id": "1"}}]))
    ...
    def test_invalid_account_refs_are_rejected(self):
        with self.assertRaises(google.GoogleAccountError):
            google.collect("bad", "2026-01-01", "2026-01-02", ("x", "y"))
