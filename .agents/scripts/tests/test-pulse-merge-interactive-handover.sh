...
setup_test_env() {
	...
	# Mock gh. Returns canned responses from TEST_ROOT state files and
	# honours the --jq filter like real gh does (server-side jq simulation).
...
case "$_subcmd" in
"pr view")
	# Labels-only lookup (used by is_stale gate 1 and trigger_handover idempotence).
	# No `local` keywords — this script runs in its own process, variables are
	# naturally scoped to the invocation.
	if [[ "$*" == *"--json labels,updatedAt"* ]]; then
		...
		_payload="{\"labels\":${_labels_json},\"updatedAt\":\"${_updated}\"}"
		if [[ -n "$_jq_filter" ]]; then
			jq -r "$_jq_filter" <<<"$_payload"
		else
			printf '%s\n' "$_payload"
		fi
		exit 0
...
if [[ "${1:-}" == "api" ]]; then
	...
	if [[ "$*" == *"/issues/"* && "$*" != *"/comments"* ]]; then
		...
		# shape that the helper's jq filter operates on.
		_name_array=$(printf '%s' "$_labels_json" | jq -c '[.[]] | map({name: .})' 2>/dev/null || echo '[]')
		_payload=$(jq -n --arg state "$_state" --argjson labels "$_name_array" \
