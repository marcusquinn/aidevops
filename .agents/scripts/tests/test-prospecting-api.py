...
from prospecting_mcp import APIClient, MCPAdapterError, _RejectRedirect  # noqa: E402
...
class ProspectingAPITest(unittest.TestCase):
    ...
    def test_read_key_cannot_mutate_and_revocation_is_immediate(self) -> None:
        headers = {**self.read_headers, "Host": "127.0.0.1:8765", "Origin": "http://127.0.0.1:8765"}
        mutation = self.request("PATCH", "/v1/operator/projects/project-alpha/leads/ask-1", headers,
                                {"disposition": "saved", "expected_version": 1})
        self.assertEqual(401, mutation.status)
        lead = self.request("GET", "/v1/projects/project-alpha/leads/ask-1", self.read_headers)
        self.assertEqual("new", lead.body["lead"]["disposition"])
    ...
    def test_owner_requires_origin_csrf_and_scope(self) -> None:
        ...
        self.assertEqual(403, self.request("PATCH", path, {**self.owner_headers, "Host": "[invalid"}, value).status)
    ...
    def test_mcp_client_rejects_ssrf(self) -> None:
        ...
        self.assertIsNone(_RejectRedirect().redirect_request(None, None, 302, "Found", {}, "https://example.test"))

    def test_rotation_requires_live_source_and_revokes_atomically(self) -> None:
        old_id = self.read_token.split("_", 2)[1]
        replacement = auth.issue_read_key(self.auth_database, ["project-alpha"], rotate=old_id)
        self.assertEqual(401, self.request("GET", "/v1/projects", self.read_headers).status)
        self.assertEqual(200, self.request("GET", "/v1/projects", {"Authorization": f"Bearer {replacement}"}).status)
        with self.assertRaises(auth.AuthError):
            auth.issue_read_key(self.auth_database, ["project-alpha"], rotate="missing")
...
class ProspectingHTTPIntegrationTest(unittest.TestCase):
    def test_isolated_http_service_and_mcp_adapter(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            process = subprocess.Popen(  # nosec B603: fixed local helper under test, no shell
            ...
            try:
                ...
                for _attempt in range(30):
                    try:
                        with urlopen(url + "/v1/health", timeout=0.2) as response:  # nosec B310: fixed loopback test URL
                ...
                with self.assertRaises(HTTPError) as denied:
                    urlopen(traversal, timeout=1)  # nosec B310: fixed loopback test URL
                self.assertEqual(404, denied.exception.code)
            finally:
                process.terminate()
                process.wait(timeout=3)
                self.assertFalse((root / "service-state.json").exists())
