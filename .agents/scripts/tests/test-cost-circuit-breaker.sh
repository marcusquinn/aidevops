...
if grep -q 'sudo aidevops approve issue 18002 owner/repo' "$STUB_LOG" 2>/dev/null && \
	grep -q '```bash' "$STUB_LOG" 2>/dev/null; then
	print_result "cost breaker comment includes copy-paste approval command" 0
else
	print_result "cost breaker comment includes copy-paste approval command" 1 \
		"(stub_log=$(tr '\n' ' ' <"$STUB_LOG" 2>/dev/null))"
fi
