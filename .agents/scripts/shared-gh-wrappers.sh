...
if [[ -n "$_SHARED_GH_WRAPPERS_DIR" && -f "$_SHARED_GH_WRAPPERS_DIR/../configs/pulse-rate-limit.conf" ]]; then
	if [[ -z "${AIDEVOPS_GH_READ_TIMEOUT+x}" ]] && [[ -z "${AIDEVOPS_GH_WRITE_TIMEOUT+x}" ]]; then
		_SHARED_GH_PULSE_BATCH_SEARCH_LAST_RESORT_WAS_SET=0
		_SHARED_GH_PULSE_BATCH_SEARCH_LAST_RESORT_SAVED=""
		if [[ -n "${PULSE_BATCH_SEARCH_LAST_RESORT+x}" ]]; then
			_SHARED_GH_PULSE_BATCH_SEARCH_LAST_RESORT_WAS_SET=1
			_SHARED_GH_PULSE_BATCH_SEARCH_LAST_RESORT_SAVED="$PULSE_BATCH_SEARCH_LAST_RESORT"
		fi
		# shellcheck disable=SC1091
		source "$_SHARED_GH_WRAPPERS_DIR/../configs/pulse-rate-limit.conf"
		if [[ "$_SHARED_GH_PULSE_BATCH_SEARCH_LAST_RESORT_WAS_SET" == "1" ]]; then
			PULSE_BATCH_SEARCH_LAST_RESORT="$_SHARED_GH_PULSE_BATCH_SEARCH_LAST_RESORT_SAVED"
		fi
		unset _SHARED_GH_PULSE_BATCH_SEARCH_LAST_RESORT_WAS_SET _SHARED_GH_PULSE_BATCH_SEARCH_LAST_RESORT_SAVED
	fi
fi
