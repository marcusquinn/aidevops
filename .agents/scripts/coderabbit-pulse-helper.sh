...
print_info() {
	local msg="$1"
	echo -e "${CR_BLUE}[PULSE]${CR_NC} $msg"
	return 0
}
print_success() {
	local msg="$1"
	echo -e "${CR_GREEN}[PULSE]${CR_NC} $msg"
	return 0
}
print_warning() {
	local msg="$1"
	echo -e "${CR_YELLOW}[PULSE]${CR_NC} $msg"
	return 0
}
print_error() {
	local msg="$1"
	echo -e "${CR_RED}[PULSE]${CR_NC} $msg"
	return 0
}
...
run_gh_api_review() {
	...
	# In cron environments, gh auth status fails (no keyring access) even when
	# GH_TOKEN is set. Check the env var first, then fall back to gh auth status.
	if [[ -z "${GH_TOKEN:-}" ]] && ! gh auth status &>/dev/null 2>&1; then
		print_error "gh CLI not authenticated (no GH_TOKEN and gh auth status failed)"
		return 1
	fi
