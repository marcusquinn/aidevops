...
def _authorize_operation_uncommitted(
    ...
    *,
    ...
    community = validate_opaque(community, "community")
    ...
    if project_id != grant.project_id or corpus_id != grant.corpus_id:
    ...
    if not grant.not_before <= current_time < grant.expires_at:
        raise SocialStoreError("delegated grant is not currently valid")
    if rules_observed_at > current_time or current_time - rules_observed_at > grant.rules_max_age_seconds:
        raise SocialStoreError("community rules evidence is stale")
    if source_observed_at > current_time:
        raise SocialStoreError("source evidence timestamp is invalid")
    ...
    database.execute(
        ...
        (authorization_id, operation_id, grant.grant_id, grant.revision, grant.policy_hash,
         ...
         community, rules_observed_at, source_observed_at, current_time),
    )
    database.execute("UPDATE outbound_operations SET state='approved',updated_at=? WHERE operation_id=? AND state='draft'", (current_time, operation_id))
    ...
    **scope: Any,
    ...
    try:
        result = _authorize_operation_uncommitted(
            database, operation_id, grant_id, **scope
        )
        database.execute("COMMIT")
        return result
    except Exception:
...
def delegated_authorization(database: sqlite3.Connection, operation_id: str, current_time: int) -> tuple[sqlite3.Row, Grant] | None:
    ...
    valid = (
        grant_row["state"] == "active" and grant.enabled
        and grant.revision == auth["grant_revision"] and grant.policy_hash == auth["grant_hash"]
        and operation["intent_sha256"] == auth["intent_sha256"]
        and grant.not_before <= current_time < grant.expires_at
        and current_time - auth["rules_observed_at"] <= grant.rules_max_age_seconds
        and database.execute("SELECT 1 FROM public_engagement_suppressions WHERE active=1 AND (account_id=? OR community=? OR thread_id=?) LIMIT 1", (grant.account_id, auth["community"], operation["target_remote_id"] or "")).fetchone() is None
    )
    if not valid:
