...
### CI failure pattern detection (GH#2973, GH#4480)
...
**Pre-mined data available:** The pre-fetched state includes a "CI Failure Patterns" section generated by `gh-failure-miner-helper.sh`. This mines `ci_activity` notifications from the last 24 hours and identifies checks that fail on 3+ PRs. Use this data as a starting point — it catches patterns from notification metadata without requiring per-PR API calls. If the miner flagged systemic patterns, act on them directly (skip per-PR dispatch, file workflow issues). For patterns not caught by the miner (e.g., failures on PRs without recent notifications), fall back to the manual correlation below.
