...
#
# Stable utilities (DB, auth, backoff, model choice, watchdog, ledger, contract,
# canary, server detection, cmd builders, metrics, failure reporting) live in
# headless-runtime-lib.sh. This helper retains the cmd_* dispatch entry points
# and the run-attempt orchestration that ties them together.
...
# =============================================================================
# Subcommand: run — argument parsing and validation
# =============================================================================
...
# =============================================================================
# Subcommand: run — runtime invocation (opencode + claude)
# =============================================================================
...
# worker to hang indefinitely waiting for an API response.
_invoke_opencode() {
	local output_file="$1"
	...
	# including the session database. GH#17549: each worker gets its OWN SQLite
	# DB (no shared OPENCODE_DB) — concurrent workers with busy_timeout=0 cause
	# SQLITE_BUSY which silently kills streaming connections. Session stats are
	# sacrificed for reliability.
	...
	# tee stdout would contaminate the $() capture.
	(
		set +e
	...
	# Activity watchdog: GH#17648: launch as a STANDALONE process via nohup,
	# not a backgrounded function. Backgrounded functions die silently when
	# nohup changes the subshell's process group — stalled workers sat forever.
	...
	if [[ -x "$_watchdog_script" ]]; then
		...
		# Fallback: use inline function from the lib if standalone script is missing
...
# =============================================================================
# Subcommand: run — result handling and attempt orchestration
# =============================================================================
...
# Returns: 0 success, 75 no-activity backoff, 76 retry after pool recovery,
#          77 premature exit, 78 watchdog stall continue, non-zero on failure.
_handle_run_result() {
	local exit_code="$1"
	local output_file="$2"
	local role="$3"
...
# Returns: 0 success, 75 no-activity, 76 pool recovery, 77 premature exit,
#          78 watchdog stall, non-zero on hard failure.
# Sets caller variable _run_failure_reason on failure.
_execute_run_attempt() {
	...
	if [[ "$role" == "pulse" ]]; then
		...
		# conversational context. Workers still keep session reuse.
...
# =============================================================================
# Subcommand: run — finish, prepare, retry, detach
# =============================================================================
...
# =============================================================================
# Subcommand: run — main entry point
# =============================================================================
