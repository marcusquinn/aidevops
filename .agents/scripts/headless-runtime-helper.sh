...
# CLI subcommands (cmd_select, cmd_backoff, cmd_session, cmd_metrics) and
# argument parsing helpers (_parse_run_args, _validate_run_args).
# shellcheck source=./headless-runtime-commands.sh
# shellcheck disable=SC1091  # sub-library resolved at runtime via $SCRIPT_DIR
source "${SCRIPT_DIR}/headless-runtime-commands.sh"

# Worker lifecycle helpers: auth rotation, rate-limit monitoring, Claude CLI
# invocation, output preservation, orphan handling, run preparation, and stall caps.
# Note: _worker_produced_output stays in this orchestrator (101 lines, must
# preserve identity key per reference/large-file-split.md §3).
...
# (_worker_produced_output stays here: 101 lines, preserves (file,fname) identity key
# per reference/large-file-split.md §3. Lifecycle helpers moved to
# headless-runtime-worker.sh which is sourced above.)
# =============================================================================
