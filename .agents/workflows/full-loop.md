...
- **Maintainer gate pre-check (GH#17810/GH#22854 — MANDATORY):** Verify issue lacks `needs-maintainer-review`. Headless workers must also verify the issue has an assignee. OWNER/MEMBER interactive sessions treat a missing assignee as actionable state: `full-loop-helper.sh start` claims/self-assigns the open issue and continues. Exit BLOCKED only for the enforced gate in the current mode.
