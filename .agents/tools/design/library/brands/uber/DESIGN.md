# Design System: Uber

Uber — confident minimalism: true black/white duality, pill-shaped everything (999px radius), UberMove Bold headlines, whisper-soft shadows, warm human illustrations contrasting stark monochrome.

## Chapters

| File | Contents |
|------|----------|
| [01-visual-theme.md](01-visual-theme.md) | Visual identity, atmosphere, key characteristics |
| [02-color-palette.md](02-color-palette.md) | Primary, interactive, text, border, shadow, link colors; gradient policy |
| [03-typography.md](03-typography.md) | Font families, type scale table, typographic principles |
| [04-components.md](04-components.md) | Buttons, cards, inputs, navigation, image treatment, distinctive components |
| [05-layout.md](05-layout.md) | Spacing system, grid, whitespace philosophy, border radius scale |
| [06-depth-elevation.md](06-depth-elevation.md) | Elevation levels table, shadow philosophy |
| [07-dos-donts.md](07-dos-donts.md) | Do/Don't rules for Uber brand compliance |
| [08-responsive.md](08-responsive.md) | Breakpoints, touch targets, collapsing strategy, image behavior |
| [09-agent-prompt-guide.md](09-agent-prompt-guide.md) | Quick color reference, example component prompts, iteration guide |

## Quick Reference

- **Primary CTA**: Uber Black (`#000000`)
- **Background**: Pure White (`#ffffff`)
- **Secondary text**: Body Gray (`#4b4b4b`), Tertiary: Muted Gray (`#afafaf`)
- **Chips/filters**: Chip Gray (`#efefef`) background
- **Font**: `UberMove` (headings, weight 700) + `UberMoveText` (body/UI, weight 400-500)
- **Radius**: 999px all buttons/pills; 8px cards; 12px featured cards; 50% circles
- **Shadow**: `rgba(0,0,0,0.12) 0px 4px 16px` standard; max opacity 0.16
- **No gradients** — every surface is a flat solid color
