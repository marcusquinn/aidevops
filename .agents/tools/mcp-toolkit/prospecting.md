...
rejected unless `--allow-external`, an exact `--public-host`, a TLS certificate,
and a TLS key are all explicitly supplied. Owner Origin checks use HTTPS and the
configured host in this mode. That technical gate is not a public-launch approval.
