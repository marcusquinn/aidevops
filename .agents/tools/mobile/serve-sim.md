...
| Reload React Native/Expo bundle | Press `R` or use Reload in the preview | Sends plain `R`; do not use `Cmd+R`, which is not the registered native reload command |
...
`serve-sim/middleware` can mount the preview under an existing Connect-style server, including Expo Metro. This is useful when a mobile dev server should expose the simulator at a stable path such as `/.sim`. Recent upstream versions route the preview, same-origin helper endpoints (including `/helper/<device>/camera/status`), and WebKit DevTools proxying through the in-process server when the host wires HTTP upgrade handling, so remote viewers only need the preview server port. The camera status endpoint reads persisted helper state without opening a simulator capture session; use `GET` and avoid polling it through a host-side `serve-sim camera status` subprocess. The device grid is paged by default and exposes total/loaded counts; searching intentionally loads the full catalog, then clearing search resets to the paged window.
