...
- **Skills**: [Official](https://github.com/tddworks/asc-cli-skills) (27 command-group skills, checked at `6465c10feb89`) | [Community](https://github.com/rudrankriyam/app-store-connect-cli-skills) (23 workflow skills for the separate community CLI, checked at `c77169a`)
...
Install on-demand (not pre-loaded): **Official** `asc skills install --all` (per-command reference) | **Community** `asc install-skills` or `npx skills add rudrankriyam/app-store-connect-cli-skills` (workflow orchestration: releases, ASO, localization, RevenueCat, crash triage, Apple Ads). The community pack targets the separate [rorkai/App-Store-Connect-CLI](https://github.com/rorkai/App-Store-Connect-CLI), not the primary `tddworks/asc-cli` documented above, so verify its CLI version before following its commands. These upstream skill packs remain on-demand rather than being imported into aidevops. Latest reviewed official skill change adds build export-compliance handling. Latest community refresh (`c77169a`) aligns its workflows with community CLI 3.1.0 and App Store Connect API 4.4.1: it adds idempotent version-scoped subscription, subscription-group, and IAP localization/review flows; requires strict validation before RevenueCat mapping even when no writes are needed; moves initial availability setup to the public API; and documents Xcode export-option generation plus atomic remote-safe build-number updates.
