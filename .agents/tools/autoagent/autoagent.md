...
- **Program format**: `.agents/templates/autoagent-program-template.md` (t1871)
...
- **Metric**: `autoagent-metric-helper.sh` (t1867) — composite score command
...
| `HYPOTHESIS_TYPES` | `## Autoagent` section, `hypothesis_types:` line (comma-separated) |
| `SIGNAL_SOURCES` | `## Autoagent` section, `signal_sources:` line (comma-separated) |
| `SAFETY_LEVEL` | `## Autoagent` section, `safety_level:` line (`standard` \| `elevated`) |
...
**1.5 Register with mailbox (multi-dimension only):** If CAMPAIGN_ID set: `mail-helper.sh register --agent "autoagent-${PROGRAM_NAME}-${DIMENSION:-solo}"`

**1.6 Mine signals from enabled sources:** Load `autoagent/signal-mining.md`. For each source in SIGNAL_SOURCES, extract findings → SIGNAL_FINDINGS list.

**1.7 Load safety constraints:** Load `autoagent/safety.md`. Determine SAFETY_LEVEL (default: `standard`). Build NEVER_MODIFY and ELEVATED_ONLY file lists.

**1.8 Measure baseline (first run only):** If `BASELINE == null`: run all constraints (fail → exit); run METRIC_CMD → `BASELINE = BEST_METRIC`; update program file `baseline: {value}`; append baseline row to results.tsv.
...
while true:
    ...
    # Safety gate: reject hypothesis if it touches NEVER_MODIFY files
    # or ELEVATED_ONLY files when SAFETY_LEVEL == "standard"
    if safety_blocked(hypothesis): continue
    ...
    if is_improvement(metric_result, BEST_METRIC, METRIC_DIR):
        # Regression gate: verify ALL comprehension tests still pass
        if regression_check() == FAIL:
            git -C WORKTREE_PATH reset --hard HEAD
            log_result(ITERATION_COUNT, null, null, "regression_fail", hypothesis, ITER_TOKENS)
            continue
...
| Regression failures | {regression_fail_count} |
| Crashes | {crash_count} |
...
**3.5 Worktree cleanup:** Return to canonical repo, pull, remove worktree.

## Logging, Memory & Mailbox

Reuses the same TSV schema as autoresearch. See `autoresearch/logging.md` for column definitions and memory storage commands.

## Related

`.agents/tools/autoresearch/autoresearch.md` · `.agents/reference/self-improvement.md` · `autoagent/signal-mining.md` · `autoagent/hypothesis-types.md` · `autoagent/safety.md` · `autoagent/evaluation.md`
