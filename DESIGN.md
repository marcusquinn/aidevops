...
- The approved headline is exactly **Designed for speed, token-efficiency, security, and infinite scaling of AI teamwork**. `docs/assets/og-copy.svg` provides an independent copy-only overlay using the original website SVG's background definitions. Keep the headline on two lines, breaking after `security,`, with 33px type and the existing left alignment. Centre the 16.5px top-line at `x=600` with `text-anchor="middle"` and a 1036px text length aligned to the card width.
- Render the copy overlay with the same transparent browser procedure as the statistics overlay below. For copy-only edits, changed pixels must be confined to the union of `(70,35,1060,40)` and `(256,228,862,84)` rectangles; logo, title, subtitle, statistics, installation command and footer pixels remain unchanged. Do not apply the card-only difference bounds to headline edits.
