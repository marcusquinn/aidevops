...
export function activeSidebarSessionId(sessions: GuiStatusData["opencode_sessions"]["sessions"], selectedSessionId: string | undefined): string | undefined {
  return sessions.some((session) => session.id_ref === selectedSessionId) ? selectedSessionId : sessions[0]?.id_ref;
}
...
function ConversationSidebar({ conversationMode, selectedLocalRepoIndex, selectedSessionId, setConversationMode, setSelectedLocalRepoIndex, setSelectedSessionId, status }: {
  ...
  const activeSessionId = activeSidebarSessionId(sessions, selectedSessionId);
