...
export function renderDashboardHtml(status: GuiResponseEnvelope<GuiStatusData>): string {
  ...
  return [
    ...
    `<p>Theme follows system preferences with light and dark overrides. Sidebar modes: DevOps and Comms. Appearance controls can be hidden or shown, and include editable Hue, Contrast levels low, medium, high, icon Reset, Show borders toggle, Show counts toggle, Font size choices xs, s, m, lg, xl, and Font options: Cal Sans, Courier Prime, DM Mono, DM Sans, Fredoka, IBM Plex Mono, IBM Plex Sans, IBM Plex Serif, Inter (default), Menlo, Mohave, Playpen Sans, Poppins, Source Sans, Source Serif, Tilt Neon, Ubuntu, Ubuntu Mono, Zilla Slab. The workspace header includes a command palette, reserved signposts/help slot immediately left of notifications, notifications menu, AI Assistant panel, and profile menu with Settings, Theme, Language, Community, Admin, and Logout entries. Notifications reuse the OpenCode toast status cache and GUI readiness data. A desktop status bar shows local readiness, repo totals, secret reference count, provider accounts, and update state.</p>`,
