...
export function renderDashboardHtml(status: GuiResponseEnvelope<GuiStatusData>): string {
  ...
  return `<section aria-label="aidevops status"><h1>aidevops control plane</h1><p>Read-only local app shell.</p><p>Inset sidebar navigation, sticky workspace header, setup card, and grouped aidevops surfaces.</p><p>Theme follows system preferences with light and dark overrides.</p><p>${escapeHtml(status.data.update.message)}</p><dl><dt>Version</dt><dd>${escapeHtml(status.data.aidevops_version)}</dd><dt>API</dt><dd>${escapeHtml(status.data.runtime.api)}</dd></dl><h2>Sidebar navigation</h2><ul><li>Dashboard</li><li>Projects</li><li>Agents</li><li>Routines</li><li>Integrations</li><li>Settings</li><li>Security</li></ul><h2>Path health</h2><ul>${paths}</ul><h2>Projects</h2><ul>${repos}</ul><h2>Settings</h2><ul>${settings}</ul><h2>Helper availability</h2><ul>${helpers}</ul><h2>Agents</h2><ul>${capabilities}</ul><h2>Secret references</h2><ul>${secrets}</ul><p>${escapeHtml(status.data.placeholders[0] ?? "More read-only adapters are planned.")}</p></section>`;
}
