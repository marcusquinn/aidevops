...
import { buildInsights } from "./status-pulse-workers-insights";
...
export function readPulseWorkersSummary(options: PulseWorkersAdapterOptions = {}): { summary: GuiPulseWorkerSummary; source_path_refs: string[] } {
  ...
  const insightWarnings = insights.map((finding) => ({
    id: finding.id,
    severity: finding.severity,
    title: finding.title,
    detail: `${finding.detail} Recommended fix: ${finding.recommendation}`,
    event_ref: finding.event_refs[0] ?? null,
  }));
  const warnings = [
    ...buildAttention(sourceStates, pulseCounters, resourceSnapshots, oauthPool.value),
    ...insightWarnings,
  ].slice(0, 12);
