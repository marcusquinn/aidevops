...
This document is the durable product and architecture memory for the planned
aidevops GUI. It captures the working decisions from the initial design session
so future agents can continue without rediscovering the mission, boundaries, or
product shape.

## Mission

Build the GUI for aidevops as the place to create, manage, oversee, and scale
the infrastructure needed to build apps, run AI agents, operate personal and
business systems, and reduce repetitive operational chores.

The app turns the aidevops agents, workflows, services, and references into a
navigable control plane: opinionated, open-source-first, freedom-preserving,
fast, affordable, scalable, and time-efficient.
...
Use a web-first TypeScript stack with a local daemon/API boundary and optional
...
| App shell | Vite or Next.js, decided by ADR | Keep the first phase evidence-based |
| API server | Node.js TypeScript with Hono or Fastify | Typed local/Cloudron API, simple deployment |
