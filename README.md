...
[![Version](https://img.shields.io/badge/Version-3.34.23-blue.svg)](https://github.com/marcusquinn/aidevops/releases)
...
- Explore optional [Jev structured decisions](.agents/tools/ai-assistants/jev.md)
  with [reversible retrieval and shadow-triage pilots](.agents/tools/ai-assistants/jev-pilots.md),
  private evaluation reports and bounded continuation examples; no default routing,
  runtime hooks or compaction changes.
...
OpenCode defaults GPT-6 Sol/Luna and their Fast variants to a ~240K usable-input
budget, unless a model's context/input limit was explicitly configured. Run
`aidevops gpt6-context enable` to force the budget, `status` to check a fresh
process, or `disable` to restore native metadata. Restart OpenCode after changes.
